...
local WIDE_FONT_THEMES = { housingtheme = true, accessibilityhc = true }
...
function VE.Constants:GetFontFile()
    ...
    if VE.Store and VE.Store.state and VE.Store.state.config then
        family = VE.Store.state.config.fontFamily
    elseif VE_DB and VE_DB.config and VE_DB.config.fontFamily then
        family = VE_DB.config.fontFamily
    end
    if not family then -- no explicit user choice; pick a theme-aware default
        if not NON_LATIN_LOCALES[GetLocale()] then
            ...
            if WIDE_FONT_THEMES[theme] then family = "Expressway" end
        end
        family = family or DEFAULT_FONT
    end
...
VE.Constants.PENDING_COMPLETION_TTL = 5
