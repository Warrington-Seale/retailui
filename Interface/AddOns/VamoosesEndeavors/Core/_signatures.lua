...
VE.EndeavorTracker:GetActivityLogData()              -- Returns cached activity log info or nil
...
-- XP / per-task API (folded in from VE_XPEngine in v1.16):
VE.EndeavorTracker:GetHouseXP()                      -- C_NeighborhoodInitiative.GetAvailableHouseXP, with nil-safety
VE.EndeavorTracker:GetPlayerContribution()           -- Sum of current player's amount entries
VE.EndeavorTracker:GetChest()                        -- Final-milestone chest: {available, claimed, xp, currencies, rewardQuestID}
VE.EndeavorTracker:GetProjectedHouseXP()             -- (projected, fromTasks, chestBonus); chest only added when available + unclaimed
VE.EndeavorTracker:CalculateNextContribution(taskName)  -- Trusts Blizzard's progressContributionAmount (already DR-applied)
VE.EndeavorTracker:GetDRStepInfo(task)               -- {step, multiplier, atFloor} for repeatable tasks; nil for Single
VE.EndeavorTracker:GetTaskRankings()                 -- Top 3 incomplete repeatable tasks by next XP

-- Housing API (folded in from VE_HousingTracker in v1.16):
VE.EndeavorTracker:RequestHouseInfo(levelOnly)       -- Request house list (or just favor if levelOnly)
VE.EndeavorTracker:UpdateCoupons()                   -- Fetch and dispatch community coupon count

-- Internal (called from RefreshActivityLogCache + housing event handlers):
-- VE.EndeavorTracker:_RebuildXPIndexes()              -- Rebuild charContributions + playerContribution from log
-- VE.EndeavorTracker:_OnHouseLevelFavorUpdated(info)  -- Dispatch SET_HOUSE_LEVEL with maxLevel/xpForNextLevel
-- VE.EndeavorTracker:_TrackCouponGain(amount, src)    -- Correlate coupon gain with most recent task completion
