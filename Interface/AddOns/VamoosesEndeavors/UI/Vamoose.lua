...
local function PlayTalkingHead(quote)
    ...
    -- PlayerModel methods can taint during combat
    ...
    if VamooseHead.hideTimer then VamooseHead.hideTimer:Cancel() end
    VamooseHead.timer = C_Timer.NewTimer(5, function()
        UIFrameFadeOut(VamooseHead, 0.5, 1, 0)
        VamooseHead.hideTimer = C_Timer.NewTimer(0.5, function()
            VamooseHead.hideTimer = nil
            VamooseHead:Hide()
        end)
    end)
end
