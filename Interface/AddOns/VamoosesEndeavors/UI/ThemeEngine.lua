...
-- Bumped whenever anything ApplyFont depends on changes (theme, font family,
-- font scale). Pooled scrollbox rows stamp the generation they last fonted at
-- and skip SetFont while it matches -- SetFont re-measures the string, so
-- re-applying it on every data refresh was the bulk of a feed repaint.
VE.Theme.fontGeneration = 1
...
function VE.Theme:UpdateAll()
    -- Single chokepoint for every font-affecting change: the VE_THEME_UPDATE
    -- listener above routes theme/font-family switches here, and the font-scale
    -- buttons call it directly. Bumping here invalidates pooled rows' cached
    -- font stamps. ThemeEngine registers its listener at ADDON_LOADED, before
    -- any tab exists, so this runs before the tabs' own repaint handlers.
    self.fontGeneration = self.fontGeneration + 1
