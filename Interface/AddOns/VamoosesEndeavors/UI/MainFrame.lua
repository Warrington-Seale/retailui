...
-- Midnight secret-value guard. See Reference/MIDNIGHT_SECRET_VALUES.md.
-- Anything coming from C_NeighborhoodInitiative / C_Housing inside a player's
-- own house can be secret-restricted; we render "--" instead of crashing.
...
function VE:CreateMainWindow()
    ...
    -- Escape closes window (unless pinned)
    if not VE.Store:GetState().config.pinWindow then
    ...
    if tabWidth then
        ...
        endeavorsTabBtn:SetSize(VE.UI.NaturalTextWidth(endeavorsTabBtn.label, nil, 50) + tabPadding, tabHeight)
    end
    ...
    if tabWidth then
        ...
        leaderboardTabBtn:SetSize(VE.UI.NaturalTextWidth(leaderboardTabBtn.label, nil, 50) + tabPadding, tabHeight)
    end
    ...
    if tabWidth then
        ...
        activityTabBtn:SetSize(VE.UI.NaturalTextWidth(activityTabBtn.label, nil, 50) + tabPadding, tabHeight)
    end
    ...
    if tabWidth then
        ...
        infoTabBtn:SetSize(VE.UI.NaturalTextWidth(infoTabBtn.label, nil, 50) + tabPadding, tabHeight)
    end
    ...
    if tabWidth then
        ...
        configTabBtn:SetSize(VE.UI.NaturalTextWidth(configTabBtn.label, nil, 50) + tabPadding, tabHeight)
    end
    ...
    function frame:UpdateHousingDisplay()
        ...
        -- so the coupon icon loads for players who happen to have 0 coupons.
        if not coupons or not couponsIconID then
            if C_CurrencyInfo and C_CurrencyInfo.GetCurrencyInfo then
                ...
                if currencyInfo then
                    ...
                    couponsIconID = currencyInfo.iconFileID or couponsIconID
                end
            end
        end
        ...
        if level > 0 then
            ...
            local xpSecret = issecretvalue(xp) or issecretvalue(xpForNextLevel)
            if level >= maxLevel then
                self.houseLevelText:SetText(string.format("%s |cFF%sLv %d|r |cFF%s(Max)|r", houseIcon, accentHex, level, dimHex))
            elseif xpSecret then
                -- Inside player housing the XP fields can come back secret-restricted.
                -- Drop the X/Y XP suffix rather than format-crashing the whole header.
                self.houseLevelText:SetText(string.format("%s |cFF%sLv %d|r", houseIcon, accentHex, level))
            else
    ...
    contribHover:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("Total this cycle across all your characters.", 0.7, 0.7, 0.7, true)

        local chars = VE.EndeavorTracker and VE.EndeavorTracker:GetCharContributions() or {}
        local sorted = {}
        for name, amt in pairs(chars) do sorted[#sorted + 1] = { name = name, amount = amt } end
        table.sort(sorted, function(a, b) return a.amount > b.amount end)

        if #sorted > 0 then
            GameTooltip:AddLine(" ")
            for _, e in ipairs(sorted) do
                GameTooltip:AddDoubleLine(e.name, string.format("%.1f", e.amount), 1, 1, 1, 0.7, 0.85, 1)
            end
        end
    ...
    houseXpIcon:SetAtlas("housing-dashboard-icon-xp")
    ...
    houseXpHover:SetScript("OnEnter", function(self)
        local colors = VE.Constants:GetThemeColors()
        GameTooltip:SetOwner(self, "ANCHOR_TOP")
        GameTooltip:AddLine("House XP Available", 1, 1, 1)
        GameTooltip:AddLine("Endeavor XP you can still earn this cycle.", 0.7, 0.7, 0.7, true)

        GameTooltip:AddLine(" ")
        local available = VE.EndeavorTracker:GetHouseXP()
        local housing = VE.Store:GetState().housing
        local curFavor = housing.xp
        -- exception(boundary): house favor can be secret-restricted / not-yet-loaded
        -- inside a housing interior; render "--" rather than tostring-crashing (mirrors header).
        local curKnown = housing.level > 0 and not issecretvalue(curFavor)
        GameTooltip:AddDoubleLine(
            "Current house XP:",
            curKnown and tostring(curFavor) or "--",
            0.7, 0.7, 0.7,
            colors.endeavor.r, colors.endeavor.g, colors.endeavor.b
        )
        -- available can be nil when the underlying API value is secret-restricted
        -- (housing interior); render "--" rather than tostring-crashing.
        GameTooltip:AddDoubleLine(
            "From tasks:",
            available and tostring(available) or "--",
            0.7, 0.7, 0.7,
            colors.endeavor.r, colors.endeavor.g, colors.endeavor.b
        )
        local chest = VE.EndeavorTracker:GetActiveChest()
        local chestForTotal = 0  -- unclaimed chest XP still earnable this cycle
        if chest.xp > 0 then
            local label, valColor
            if chest.claimed then
                label, valColor = "Completion chest:", { 0.5, 0.5, 0.5 }
                GameTooltip:AddDoubleLine(label, tostring(chest.xp) .. " (claimed)", 0.7, 0.7, 0.7, valColor[1], valColor[2], valColor[3])
            elseif chest.available then
                label = "Completion chest:"
                GameTooltip:AddDoubleLine(label, "+" .. chest.xp .. " (ready to claim)", 0.7, 0.7, 0.7, colors.warning.r, colors.warning.g, colors.warning.b)
                chestForTotal = chest.xp
            else
                label = "Completion chest:"
                GameTooltip:AddDoubleLine(label, "+" .. chest.xp .. " (at 100%)", 0.7, 0.7, 0.7, 0.6, 0.6, 0.6)
                chestForTotal = chest.xp
            end
        end
        -- Projected total = house favor you'd hold after earning everything still
        -- available this cycle: current favor + remaining task XP + unclaimed chest.
        GameTooltip:AddLine(" ")
        local projectedKnown = curKnown and available
        GameTooltip:AddDoubleLine(
            "Projected total:",
            projectedKnown and tostring(curFavor + available + chestForTotal) or "--",
            1, 1, 1,
            colors.endeavor.r, colors.endeavor.g, colors.endeavor.b
        )
        GameTooltip:Show()
    ...
    function frame:UpdateHouseDropdown(houseList, selectedIndex)
        if houseList and #houseList > 0 then
            ...
            for i, houseInfo in ipairs(houseList) do
                local name = type(houseInfo.houseName) == "string" and houseInfo.houseName ~= "" and houseInfo.houseName or nil
                table.insert(items, {
                    key = i,
                    label = name or ("House " .. i),
                })
            end
            self.houseDropdown:SetItems(items)

            -- Set selected house (houseName can be 0 (number) for unnamed houses)
            local selectedHouse = houseList[selectedIndex or 1]
            local selName = selectedHouse and type(selectedHouse.houseName) == "string" and selectedHouse.houseName ~= "" and selectedHouse.houseName or nil
            self.houseDropdown:SetSelected(selectedIndex or 1, { label = selName or "Select House" })
        else
    ...
    function frame:UpdateActiveNeighborhood()
        ...
        if VE.EndeavorTracker and VE.EndeavorTracker.houseList then
            for _, houseInfo in ipairs(VE.EndeavorTracker.houseList) do
                if houseInfo.neighborhoodGUID == activeGUID then
                    -- houseName / neighborhoodName can both be 0 (number) for unnamed houses
                    local hn = type(houseInfo.houseName) == "string" and houseInfo.houseName ~= "" and houseInfo.houseName or nil
                    local nn = type(houseInfo.neighborhoodName) == "string" and houseInfo.neighborhoodName ~= "" and houseInfo.neighborhoodName or nil
                    activeName = hn or nn
                    break
                end
            end
        ...
        local colors = VE.Constants:GetThemeColors()
        local accentHex = string.format("%02x%02x%02x", math.floor(colors.accent.r*255), math.floor(colors.accent.g*255), math.floor(colors.accent.b*255))
    ...
    frame:HookScript("OnShow", function()
        ...
        if VE.EndeavorTracker then
            ...
            VE.EndeavorTracker:RequestHouseInfo(hasHouseGUID)  -- levelOnly if we have a cached house
    ...
    function frame:UpdateHeader()
        ...
        local chestState = state.endeavor.chest or { available = false, claimed = false }
        self.progressBar:SetChestState(chestState.available, chestState.claimed)
        ...
        local playerContribution = VE.EndeavorTracker:GetPlayerContribution()
        ...
        -- House XP: native via C_NeighborhoodInitiative.GetAvailableHouseXP.
        -- Returns nil when secret-restricted (housing interior); render "--"
        -- in that case rather than tostring-crashing on the secret.
        local houseXpEarned = VE.EndeavorTracker:GetHouseXP()
        self.houseXpText:SetText(houseXpEarned and tostring(houseXpEarned) or "--")
        self.houseXpAvailable = houseXpEarned
    ...
    for i = 1, MAX_MINI_ROWS do
        ...
        VE.Theme:Register(row, "Panel") -- re-skin backdrop on theme change
    ...
    -- Tab panels are built on FIRST VISIT, not up front. Constructing all five
    -- at PLAYER_LOGIN meant every session paid for panels many players never
    -- open. Only the default tab (endeavors) gets built during this function,
    -- via the ShowTab("endeavors") call below.
    --
    -- Consumers outside this file already nil-guard the tab fields (they read
    -- `VE.MainFrame.activityTab and ...`), which is what makes deferral safe.
    local TAB_FIELDS = {
        endeavors   = "endeavorsTab",
    ...
    local TAB_BUILDERS = {
        endeavors   = function() return VE.UI.Tabs:CreateEndeavors(content) end,
        leaderboard = function() return VE.UI.Tabs:CreateLeaderboard(content) end,
        activity    = function() return VE.UI.Tabs:CreateActivity(content) end,
        info        = function() return VE.UI.Tabs:CreateInfo(content) end,
        config      = function() return VE.UI.Tabs:CreateConfig(content) end,
    }

    local TAB_BUTTONS = {
        endeavors   = endeavorsTabBtn,
        leaderboard = leaderboardTabBtn,
        activity    = activityTabBtn,
        info        = infoTabBtn,
        config      = configTabBtn,
    }
    ...
    local function GetTab(tabName)
        local field = TAB_FIELDS[tabName]
        local tab = frame[field]
        if not tab then
            tab = TAB_BUILDERS[tabName]()
            tab:SetAllPoints()
            -- MUST hide on creation. CreateFrame returns a SHOWN frame, and each
            -- tab wires its own OnShow during construction -- so ShowTab's
            -- :Show() below would be a no-op on a frame that's already shown, and
            -- OnShow would never fire on first visit. That handler is what
            -- requests data and forces the first render, so the tab sat empty
            -- until some unrelated event happened to refresh it. The old
            -- eager-build path Hid every tab for exactly this reason.
            tab:Hide()
            frame[field] = tab
        end
        return tab
    ...
    local function ShowTab(tabName)
        -- Hide the tabs that exist; unbuilt ones can't be showing.
        for _, field in pairs(TAB_FIELDS) do
            local tab = frame[field]
            if tab then tab:Hide() end
        end

        -- Deactivate all buttons
        for _, btn in pairs(TAB_BUTTONS) do
            btn:SetActive(false)
        end

        GetTab(tabName):Show()
        TAB_BUTTONS[tabName]:SetActive(true)
    ...
    VE.EventBus:Register("VE_STATE_CHANGED", function(payload)
        ...
        -- Update squirrel mascot visibility and header when config changes
    ...
    VE.EventBus:Register("VE_THEME_UPDATE", function()
        ...
        -- Runs inline: this used to be deferred a frame because GetStringWidth
        -- reads 0 until the FontString has drawn, but NaturalTextWidth measures
        -- against the font directly and needs no render -- so there's no reason
        -- to bounce UI geometry through a timer.
        ...
        for i, tab in ipairs(tabs) do
            ...
            elseif tab.label then
                tab:SetWidth(VE.UI.NaturalTextWidth(tab.label, nil, 50) + padding)
            end
            if i > 1 then
    ...
    -- Hide window during combat, restore after
    local combatFrame = CreateFrame("Frame")
    combatFrame:RegisterEvent("PLAYER_REGEN_DISABLED")
    combatFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
    combatFrame:SetScript("OnEvent", function(_, event)
        -- "Show During Combat" config: leave the window exactly as the player left it
        -- through combat (no auto-hide, no restore). The frame is unprotected and
        -- updates fine mid-combat, so keeping it shown is taint-free.
        if event == "PLAYER_REGEN_DISABLED" then
            -- Checked on the way IN only. Testing the config on the way out too
            -- meant enabling "Show During Combat" while the window was already
            -- combat-hidden swallowed the restore, leaving it hidden until the
            -- player reopened it manually. If we hid it, we un-hide it.
            if VE.Store:GetState().config.showInCombat then return end
            frame._wasShownBeforeCombat = frame:IsShown()
            frame:Hide()
        elseif event == "PLAYER_REGEN_ENABLED" then
            if frame._wasShownBeforeCombat then
                frame._wasShownBeforeCombat = nil
                frame:Show()
            end
        end
    end)
