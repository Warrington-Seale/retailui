...
local function ComputeProgressHash(tasks)
    ...
    for i, task in ipairs(tasks) do
        ...
        -- Also include timesCompleted (DR step changes between reps even when
        -- task.current ends up at 0/N again) and points (Blizzard's dampened
        -- forecast) so the row redraws when forecasts change.
        hash = hash
            + i * 1000
            + (task.current or 0)
            + ((task.completed and 500) or 0)
            + (task.timesCompleted or 0) * 7
            + (task.points or 0) * 13
    end
    return hash
end
...
local function GetNextXPForTask(task)
    if not task.id or not VE.EndeavorTracker then return 0 end
    return VE.EndeavorTracker:CalculateNextContribution(task.name)
end
...
function VE.UI.Tabs:CreateEndeavors(parent)
    ...
    rewardsBtn:SetScript("OnLeave", function()
        UpdateRewardsIcon()
        GameTooltip:Hide()
    end)
    ...
    sortNextXPBtn:SetScript("OnClick", function()
        if sortState.column == "nextXP" then
    ...
    function container:ShowEmptyState()
        ...
        -- Check if the Store state indicates stale/missing endeavor data
        local state = VE.Store:GetState()
        local seasonName = state.endeavor.seasonName or ""
        local hasNoEndeavorData = seasonName == "Not Active Endeavor" or seasonName == "No Active Endeavor" or seasonName == ""

        if isViewingActive and not hasNoEndeavorData then
            -- Viewing the ACTIVE neighborhood with real data
            if isFetching then
            ...
            -- No active endeavor data - show Set as Active button to re-trigger fetch
            if hasNoEndeavorData then
                self.emptyText:SetText("No active endeavor set.\nSelect a house and set it as your active endeavor.")
            else
    ...
    -- Live refresh on task data updates (progress, completions, DR step changes).
    -- Without this the tab only re-renders on tab-switch / OnShow.
    VE.EventBus:Register("VE_STATE_CHANGED", function(payload)
        ...
        if action == "SET_TASKS" or action == "SET_ENDEAVOR_INFO" or action == "SET_HOUSE_LEVEL" then
            container:Update()
        end
    end)
