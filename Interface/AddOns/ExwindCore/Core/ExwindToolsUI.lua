...
function EXUI:CreateSidebar(parent)
    ...
    vLine:SetColorTexture(0.12, 0.15, 0.2, 0.9)
    ...
    if searchBox.SetBackdrop then
        ...
        searchBox:SetBackdropColor(0.06, 0.07, 0.09, 0.96)
        searchBox:SetBackdropBorderColor(0.2, 0.22, 0.28, 1)
    end
...
local function GetTopNavColor(page)
    if page == "Home" then
        return 0.90, 0.92, 0.98
    elseif page == "LoadSettings" then
        return 0.98, 0.84, 0.50
    elseif page == "Diagnostic" then
        return 0.24, 0.78, 1.00
    elseif page == "ProfileManager" then
        return 0.82, 0.68, 1.00
    end
    return 0.82, 0.86, 0.94
end
...
local function ApplySidebarItemLayout(btn, variant)
    ...
    if btn.variant == "topnav" then
        ...
        btn.label:SetFont(defaultFontPath, 15, "OUTLINE")
    else
        btn:SetHeight(28)
        btn.rail:Show()
        btn.accent:Show()
...
function EXUI:CreateCategoryHeaderBase(parent)
    ...
    btn.label:SetJustifyH("LEFT")
    btn.label:SetTextColor(0.97, 0.98, 1, 0.98)
    ...
    btn:SetScript("OnLeave", function(self)
        self.label:SetTextColor(0.97, 0.98, 1, 0.98)
    end)
    ...
    btn:SetScript("OnEnter", function(self)
        ...
        if not self.isActive then
            if self.variant == "topnav" then
                ...
                self.label:SetTextColor(0.83, 0.88, 0.97, 1)
                self.rail:SetColorTexture(0.34, 0.40, 0.52, 0.8)
            end
        end
    end)
    btn:SetScript("OnLeave", function(self)
        ...
        if not self.isActive then
            if self.variant == "topnav" then
                ...
                self.label:SetTextColor(0.57, 0.63, 0.75, 1)
                self.rail:SetColorTexture(0.24, 0.29, 0.38, 0.55)
            end
        end
    end)
        ...
        if btn:IsShown() and btn.page then
            ...
            if isActive then
                if btn.variant == "topnav" then
                    ...
                    btn.label:SetTextColor(0.0, 0.72, 1.0, 1)
                    btn.accent:SetAlpha(1)
                    btn.dot:SetTextColor(0.0, 0.72, 1.0, 1)
                    btn.rail:SetColorTexture(0.24, 0.29, 0.38, 0.55)
                end
            else
                btn.accent:SetAlpha(0)
                ...
                if btn.isLoaded == false then
                    ...
                    if btn.variant ~= "topnav" then
                        btn.rail:SetColorTexture(0.20, 0.22, 0.28, 0.35)
                    end
                else
                    if btn.variant == "topnav" then
                        local r, g, b = unpack(btn.topnavColor or { 0.82, 0.86, 0.94 })
