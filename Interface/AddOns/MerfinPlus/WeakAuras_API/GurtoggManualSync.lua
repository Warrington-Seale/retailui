-- Manual Gurtogg group transport for the optional WeakAura manual mode.
local MerfinPlus = LibStub("AceAddon-3.0"):GetAddon("MerfinPlus")
local PREFIX, MAX_BYTES, MAX_GROUPS, MAX_MEMBERS = "MFGURT1", 4096, 3, 10
local CHUNK_BYTES, MAX_CHUNKS = 160, 32
local incoming, outgoing = {}, {}

local function announce(message)
  local text = "|cff57d9ffMerfinPlus Manual Sync Gurtogg:|r " .. tostring(message)
...
local function normalize(snapshot)
  if type(snapshot) ~= "table" or type(snapshot.groups) ~= "table" then return nil end
  local result = {
    schema = "merfinplus.gurtogg.assignments", version = 3, available = true,
    groupOrder = { "star", "diamond", "circle" }, groups = {}, source = { mode = "weak-aura-manual" },
  }
  local expected = { star = { 1, 1 }, diamond = { 2, 3 }, circle = { 3, 2 } }
  for _, group in ipairs(snapshot.groups) do
    local layout = expected[group.key]
    if layout then
      local item = { key = group.key, order = layout[1], markerIndex = layout[2], stackTarget = math.max(1, math.min(10, tonumber(group.stackTarget) or 1)), players = {}, members = {} }
      for _, member in ipairs(group.members or group.players or {}) do
        if #item.players >= MAX_MEMBERS then return nil end
        local name = type(member) == "table" and member.name or member
        name = tostring(name or ""):match("^([^%-]+)")
        if not name or name == "" or #name > 48 then return nil end
        item.players[#item.players + 1] = name
        item.members[#item.members + 1] = { name = name, classToken = type(member) == "table" and member.classToken or nil, spec = type(member) == "table" and member.spec or nil }
      end
      result.groups[#result.groups + 1] = item
    end
  end
  return #result.groups == MAX_GROUPS and result or nil
end
...
function MerfinPlus:BroadcastGurtoggManualGroups(snapshot)
  ...
  local normalized = normalize(snapshot)
  local payload = normalized and encode(normalized)
  if not payload or #payload > MAX_BYTES then
    announce("Sync failed — manual groups could not be encoded.")
    return false, "Manual groups could not be encoded."
  end
  ...
  if chunks > MAX_CHUNKS then
    announce("Sync failed — manual groups are too large.")
    return false, "Manual group sync is too large."
  end
  ...
  outgoing[transfer] = { acknowledged = {} }
  ...
  self:PublishReceivedGurtoggManualSnapshot(normalized, UnitName("player"))
...
frame:SetScript("OnEvent", function(_, event, prefix, message, channel, sender)
  ...
  if snapshot then
    MerfinPlus:PublishReceivedGurtoggManualSnapshot(snapshot, sender)
