...
function API.OnInit(aura_env)
  ...
  aura_env.UnitBuffInfo = function(unit)
    local bestRemaining
    ...
    return true, bestRemaining <= aura_env.expiringSoonThreshold, bestRemaining
  end
  ...
  aura_env.UnitBuffIsOk = function(unit)
    local hasBuff, expiringSoon = aura_env.UnitBuffInfo(unit)
...
function API.Trigger(aura_env, states)
  if aura_env.lastCastUnit
  and aura_env.UnitBuffIsOk(aura_env.lastCastUnit)
  then
    aura_env.lastCastUnit = nil
  end
  ...
  for unit in IterateGroupMembers() do
    if aura_env.UnitIsBuffable(unit) then
      local hasRealBuff, expiringSoon, remaining = aura_env.UnitBuffInfo(unit)
