...
local encounterOrders = {
  ...
  hyjalSummit = { "618", "619", "620" },
}

local selectedRaidCooldown = {}
local transferState = { exportText = "", importText = "", status = "" }
...
local function FindRaidCooldown(raidKey, encounterID, cooldownID, npcID)
  for _, cooldown in ipairs(raidCooldownCatalog[raidKey] or {}) do
    if cooldown.encounter == encounterID and cooldown.id == cooldownID
      and (not npcID or cooldown.npc == npcID) then
      return cooldown
    end
  end
end
...
local function BuildEncounterCooldownOptions(raidKey, encounterID)
  ...
  local selectionKey = raidKey .. ":" .. encounterID
  selectedRaidCooldown[selectionKey] = selectedRaidCooldown[selectionKey]
    or (entries[1] and entries[1].id)

  local function Selected()
    return FindRaidCooldown(raidKey, encounterID, selectedRaidCooldown[selectionKey])
  end
  ...
  return {
    cooldown = {
      ...
      values = function()
        local values = {}
        for _, entry in ipairs(entries) do values[entry.id] = CooldownLabel(entry) end
        return values
      end,
    ...
    disableAll = {
      type = "toggle", name = L["Disable All Cooldowns for Boss"], order = 2, width = 1.5,
      get = Disabled,
    ...
    cooldownHeader = {
      type = "header", order = 3,
      name = function() local selected = Selected(); return selected and CooldownLabel(selected) or L["Cooldown Options"] end,
    },
    displayBar = {
      type = "toggle", name = L["Display as Bar"], order = 4.1, width = 1,
      disabled = Disabled,
      get = function() local settings = Settings(); return not settings or settings.displayBar ~= false end,
      set = function(_, value) Settings().displayBar = value; NotifyChanged() end,
    },
    displayTimeline = {
      type = "toggle", name = L["Display on Timeline"], order = 4.2, width = 1,
      disabled = Disabled,
      get = function() local settings = Settings(); return not settings or settings.displayTimeline ~= false end,
      set = function(_, value) Settings().displayTimeline = value; NotifyChanged() end,
    },
    emphasizedBar = {
      type = "toggle", name = L["Emphasized Bar"], order = 5, width = 1.25,
      disabled = Disabled,
      get = function() local settings = Settings(); return settings and settings.emphasizedBar or false end,
      set = function(_, value) Settings().emphasizedBar = value; NotifyChanged() end,
    },
    emphasizedOn = {
      type = "range", name = L["Emphasize when (sec) left"], order = 6,
      min = 3, max = 30, step = 0.5, width = 1.5,
      disabled = function() local settings = Settings(); return Disabled() or not (settings and settings.emphasizedBar) end,
      get = function() local settings = Settings(); return settings and settings.emphasizedOn or 7 end,
      set = function(_, value) Settings().emphasizedOn = value; NotifyChanged() end,
    },
    enabledCustom = {
      type = "toggle", name = L["Use Custom Name"], order = 7, width = 1.25,
      disabled = Disabled,
      get = function() local settings = Settings(); return settings and settings.enabledCustom or false end,
      set = function(_, value) Settings().enabledCustom = value; NotifyChanged() end,
    },
    customName = {
      type = "input", name = L["Custom Name"], order = 8, width = 1.75,
      disabled = function() local settings = Settings(); return Disabled() or not (settings and settings.enabledCustom) end,
      get = function() local settings = Settings(); return settings and settings.customName or "" end,
      set = function(_, value) Settings().customName = value; NotifyChanged() end,
    },
  }
end
...
local function BuildTrashCooldownOptions(raidKey, encounterID)
  ...
  local function Entries()
    local entries = {}
    for _, cooldown in ipairs(raidCooldownCatalog[raidKey] or {}) do
      if cooldown.encounter == encounterID and cooldown.npc == selectedNpc then
        entries[#entries + 1] = cooldown
      end
    end
    return entries
  ...
  local function Selected()
    ...
    selectedByNpc[selectedNpc] = selectedByNpc[selectedNpc] or (entries[1] and entries[1].id)
    return FindRaidCooldown(raidKey, encounterID, selectedByNpc[selectedNpc], selectedNpc)
  end
  local function Settings()
    local cooldown = Selected()
    if not cooldown then return end
  ...
  return {
    ...
    cooldown = {
      type = "select", name = L["Cooldown"], order = 2, width = 1.5,
      disabled = Disabled,
      values = function()
        local values = {}
        for _, entry in ipairs(Entries()) do values[entry.id] = CooldownLabel(entry) end
      ...
      get = function() local selected = Selected(); return selected and selected.id end,
      set = function(_, value) if selectedNpc then selectedByNpc[selectedNpc] = value end end,
    },
    disableAll = {
    ...
    row = { type = "description", name = "", order = 2.6, width = "full" },
    cooldownHeader = {
      type = "header", order = 3,
      name = function() local selected = Selected(); return selected and CooldownLabel(selected) or L["Cooldown Options"] end,
    },
    displayBar = {
      ...
      disabled = Disabled,
      get = function() local settings = Settings(); return settings and settings.displayBar == true end,
      set = function(_, value) Settings().displayBar = value; NotifyChanged() end,
    },
    displayTimeline = {
      type = "toggle", name = L["Display on Timeline"], order = 4.2, width = 0.9,
      disabled = Disabled,
      get = function() local settings = Settings(); return settings and settings.displayTimeline == true end,
      set = function(_, value) Settings().displayTimeline = value; NotifyChanged() end,
    },
    displayNameplate = {
      type = "toggle", name = L["Display on Nameplates"], order = 4.3, width = 1.2,
      disabled = Disabled,
      get = function() local settings = Settings(); return not settings or settings.displayNameplate ~= false end,
      set = function(_, value) Settings().displayNameplate = value; NotifyChanged() end,
    },
    emphasizedBar = {
      type = "toggle", name = L["Emphasized Bar"], order = 5, width = 1.25,
      disabled = Disabled,
...
local function BuildTransferOptions()
  return {
    generateExport = {
      ...
      func = function()
        transferState.exportText = BuildExport()
        AceConfigRegistry:NotifyChange("MerfinPlus_RaidPack")
      end,
    },
    exportString = {
      type = "input", name = L["Export String"], order = 2, width = "full", multiline = 10,
      hidden = function() return transferState.exportText == "" end,
      get = function() return transferState.exportText end,
      set = function() end,
    },
    importString = {
      type = "input", name = L["Import String"], order = 3, width = "full", multiline = 10,
      get = function() return transferState.importText end,
      set = function(_, value) transferState.importText = value or ""; transferState.status = "" end,
    },
    importButton = {
      type = "execute", name = L["Import"], order = 4, width = 1,
      disabled = function() return transferState.importText == "" end,
      func = function()
        local ok, message = Import(transferState.importText)
...
function MerfinPlus:BuildRaidCooldownOptions()
  ...
  local options = {
    type = "group",
    name = function()
      return "|TInterface\\Icons\\INV_Misc_PocketWatch_01:16:16:0:0|t " .. L["Cooldowns"]
    end,
    childGroups = "tab",
    args = {
      general = {
          ...
          hyjalSummit = {
            ...
            args = {
              empty = {
                ...
                name = L["Cooldown settings for this raid will be added later."],
              },
            },
          },
        },
