...
for _, field in ipairs({
  "type", "x", "y", "player", "label", "rotation", "bossFacingVisible", "color", "marker",
  ...
  "positionRole", "rolePosition", "specialAssignmentKey", "drawingMode", "drawingPoints", "drawingWidth",
...
local TEXT_FONTS = { Roboto = true, Inter = true, Arial = true, Georgia = true, ["Trebuchet MS"] = true, ["Courier New"] = true, Verdana = true, ["Comic Sans MS"] = true }
...
local function ValidateElement(element, planIndex, elementIndex, elementCount, totals)
  ...
  for _, field in ipairs({ "color", "fillColor", "strokeColor", "textColor", "textStrokeColor" }) do
  ...
  if not ok then return nil, errorText end
  for _, field in ipairs({
    "bossFacingVisible", "fill", "textBackdrop", "textBold", "textItalic", "textUnderline",
    "textStrikethrough", "textStroke", "drawingFadeOut", "centerDot", "locked",
  }) do
...
-- canonical MFPRA1 codec reuses its exhaustive Boss Plan validation without
