...
local FONT_ITALIC = "Interface\\AddOns\\MerfinPlus\\Media\\font\\Caveat-SemiBold.ttf"
local FONT_EXPRESSWAY = "Interface\\AddOns\\MerfinPlus\\Media\\font\\Expressway.ttf"
local FONT_MONO = "Interface\\AddOns\\MerfinPlus\\Media\\font\\HOOGE.TTF"
local FONT_PT = "Interface\\AddOns\\MerfinPlus\\Media\\font\\PTSansNarrow.ttf"
local FONT_PT_BOLD = "Interface\\AddOns\\MerfinPlus\\Media\\font\\PTSansNarrow-Bold.ttf"
local FONT_ARCHIVO = "Interface\\AddOns\\MerfinPlus\\Media\\font\\ArchivoNarrow-Bold.ttf"
...
local CANVAS_WIDTH = 1150
local CANVAS_HEIGHT = 780
...
local EDITOR_LAYOUT = {
  toolbarRegion = "below-boss-header-responsive-flow",
  persistentRightPanel = false,
  canvasHorizontalChrome = 28,
  canvasVerticalChrome = 132,
  headerRows = 2,
  actionButtonsInHeader = true,
  responsiveFlow = true,
  toolbarHeight = 54,
  minimumInspectorWidth = 440,
  paletteCapacity = 42,
}
...
local FONT_CHOICES = {
  { token = "Roboto", path = FONT_REGULAR }, { token = "Inter", path = FONT_PT },
  { token = "Arial", path = FONT_ARCHIVO }, { token = "Georgia", path = FONT_ARCHIVO },
  { token = "Trebuchet MS", path = FONT_EXPRESSWAY }, { token = "Courier New", path = FONT_MONO },
  { token = "Verdana", path = FONT_PT_BOLD }, { token = "Comic Sans MS", path = FONT_ITALIC },
}
local FONT_PATHS = {}
for _, choice in ipairs(FONT_CHOICES) do FONT_PATHS[choice.token] = choice.path end
...
MerfinPlus.MGMRA4BossPlanRendererFieldCoverage = {
  ...
  positionRole = "metadata", rolePosition = "rendered", specialAssignmentKey = "metadata",
...
local function FontPath(element)
  if element.textItalic then return FONT_ITALIC end
  if element.textBold then return FONT_BOLD end
  return FONT_PATHS[element.textFont or "Roboto"] or FONT_REGULAR
end
...
local function EnsureSelectionHandles(viewer, frame, element, sourceIndex)
  ...
  for _, spec in ipairs(specs) do
    ...
    if not handle then
      ...
      handle.inner:SetColorTexture(1, 0.76, 0.18, 1)
  ...
  if not rotate then
    ...
    rotate.icon:SetVertexColor(1, 0.76, 0.18, 1)
    frame.mgmra4EditHandles.ROTATE = rotate
    frame.mgmra4RotationStem = frame:CreateTexture(nil, "OVERLAY")
    frame.mgmra4RotationStem:SetColorTexture(1, 0.76, 0.18, 1)
  end
...
local function RenderElement(viewer, element, sourceIndex, elements)
  ...
  if element.type == "player" then
    ...
    local icon = AddImage(frame, iconPath or "Interface\\Icons\\INV_Misc_QuestionMark", 34 * (element.size or 100) / 100, 34 * (element.size or 100) / 100)
    ...
    if element.rolePosition then AddNumberOverlay(frame, element.rolePosition, icon) end
  ...
  elseif element.type == "boss" or element.type == "image" then
    local icon = AddImage(frame, ResolveIconPath(element) or "Interface\\Icons\\INV_Misc_QuestionMark", element.type == "boss" and math.min(width, height) * 0.54 or width, element.type == "boss" and math.min(width, height) * 0.54 or height, true)
    if element.type == "boss" and element.bossFacingVisible ~= false then
      local centerX, topY = width / 2, 2
      AddLine(frame, centerX, topY, centerX - 8, topY + 8, 3, "#ff4f58", 1, 3)
      AddLine(frame, centerX, topY, centerX + 8, topY + 8, 3, "#ff4f58", 1, 3)
    end
    if element.rolePosition then AddNumberOverlay(frame, element.rolePosition, icon) end
...
local function CreateViewer(owner)
  ...
  if viewer.dragCapture.SetPropagateKeyboardInput then viewer.dragCapture:SetPropagateKeyboardInput(false) end
  ...
  SetBackdrop(viewer, { 0.015, 0.02, 0.028, 0.99 }, { 0.86, 0.58, 0.08, 0.95 })
  ...
  local function Button(parent, label, width, height)
    ...
    SetBackdrop(button, { 0.05, 0.055, 0.06, 1 }, { 0.45, 0.45, 0.42, 1 })
    button.label = button:CreateFontString(nil, "OVERLAY")
    button.label:SetFont(FONT_BOLD, 11, "")
    button.label:SetPoint("CENTER")
    button.label:SetText(label)
    button:SetScript("OnEnter", function(self)
      if not self.tooltipText or not GameTooltip then return end
      GameTooltip:SetOwner(self, "ANCHOR_TOP")
      GameTooltip:SetText(self.tooltipText, 1, 0.82, 0.25)
      GameTooltip:Show()
    end)
  ...
  local function IconButton(parent, iconPath, tooltip, size)
    ...
    SetBackdrop(button, { 0.035, 0.045, 0.06, 1 }, { 0.24, 0.3, 0.38, 1 })
    ...
    button.SetActive = function(self, active)
      ...
      SetBackdrop(self, self.active and { 0.16, 0.12, 0.03, 1 } or { 0.035, 0.045, 0.06, 1 },
        self.active and { 1, 0.72, 0.12, 1 } or { 0.24, 0.3, 0.38, 1 })
      if self.iconColor then self.icon:SetVertexColor(self.iconColor[1], self.iconColor[2], self.iconColor[3], 1)
      elseif self.preserveIconColor then self.icon:SetVertexColor(1, 1, 1, 1)
      else self.icon:SetVertexColor(self.active and 1 or 0.78, self.active and 0.83 or 0.84, self.active and 0.28 or 0.9, 1) end
    end
    button:SetScript("OnEnter", function(self)
      if not GameTooltip then return end
      GameTooltip:SetOwner(self, "ANCHOR_LEFT")
      GameTooltip:SetText(self.tooltipText or "Boss Plan Tool", 1, 0.82, 0.25)
      GameTooltip:Show()
    end)
    button:SetScript("OnLeave", function() if GameTooltip then GameTooltip:Hide() end end)
    return button
  ...
  viewer.bossIcon:SetSize(36, 36)
  viewer.bossIcon:SetPoint("TOPLEFT", viewer, "TOPLEFT", 14, -10)
  viewer.bossName = viewer:CreateFontString(nil, "OVERLAY")
  viewer.bossName:SetFont(FONT_BOLD, 18, "OUTLINE")
  ...
  viewer.planName:SetFont(FONT_REGULAR, 12, "")
  ...
  viewer.close = Button(viewer, "Close", 54, 26)
  viewer.close:SetPoint("TOPRIGHT", viewer, "TOPRIGHT", -10, -10)
  ...
  viewer.restore = Button(viewer, "Restore Default", 82, 26)
  viewer.unlock = Button(viewer, "Unlock Plan", 80, 26)
  viewer.save = Button(viewer, "Save", 42, 26)
  viewer.send = Button(viewer, "Send", 42, 26)
  viewer.remove = Button(viewer, "Remove", 54, 26)
  viewer.clear = Button(viewer, "Clear", 44, 26)
  ...
  viewer.inspector:SetSize(148, 250)
  SetBackdrop(viewer.inspector, { 0.025, 0.032, 0.044, 1 }, { 0.2, 0.27, 0.35, 1 })
  ...
  SetBackdrop(viewer.textEdit, { 0.04, 0.05, 0.065, 1 }, { 0.3, 0.36, 0.44, 1 })
  ...
  SetBackdrop(viewer.fontMenu, { 0.025, 0.03, 0.04, 1 }, { 0.35, 0.42, 0.5, 1 })
  viewer.fontRows = {}
  ...
  -- Editor tools and contextual controls occupy a compact two-line header
  -- region. Their children are reflowed whenever the viewer changes width.
  viewer.toolbar = CreateFrame("Frame", nil, viewer)
  viewer.toolbar:SetHeight(EDITOR_LAYOUT.toolbarHeight)
  viewer.toolbar:SetPoint("TOPLEFT", viewer, "TOPLEFT", 14, -60)
  viewer.toolbar:SetPoint("TOPRIGHT", viewer, "TOPRIGHT", -14, -60)
  ...
  local tools = {
    ...
    { "Place boss portrait", "boss", ADDON_ROOT .. "bosses\\tbc\\serpentshrine_cavern\\hydross_the_unstable.tga" },
  ...
  for index, spec in ipairs(tools) do
    local button = IconButton(viewer.toolbarContent, spec[3], spec[1], 24)
    button:SetPoint("TOPLEFT", viewer.toolbarContent, "TOPLEFT", (index - 1) * 26, 0)
    button.tool = spec[2]
    viewer.toolButtons[#viewer.toolButtons + 1] = button
  end

  viewer.inspector:SetParent(viewer.toolbarContent)
  viewer.inspector:ClearAllPoints()
  viewer.inspector:SetPoint("TOPLEFT", viewer.toolbarContent, "TOPLEFT", 194, 0)
  viewer.inspector:SetSize(420, EDITOR_LAYOUT.toolbarHeight)
  ...
  viewer.paletteScroll:SetAllPoints(viewer.inspector)
  viewer.paletteScroll:EnableMouseWheel(true)
  viewer.paletteChild = CreateFrame("Frame", nil, viewer.paletteScroll)
  viewer.paletteChild:SetSize(1, 40)
  viewer.paletteScroll:SetScrollChild(viewer.paletteChild)
  viewer.paletteButtons = {}
  for index = 1, 42 do
    local paletteButton = IconButton(viewer.paletteChild, "Interface\\Icons\\INV_Misc_QuestionMark", "Asset", 18)
    local column, row = (index - 1) % 21, math.floor((index - 1) / 21)
    paletteButton:SetPoint("TOPLEFT", viewer.paletteChild, "TOPLEFT", 1 + (column * 20), -(row * 20))
  ...
  function viewer:LayoutInspectorControls()
    ...
    local width = math.max(1, self.inspector:GetWidth() or 1)
    local x, y, gap, rowHeight = 4, 2, 4, 22
    local controls = {
    ...
    for _, control in ipairs(controls) do
      if control:IsShown() then
        ...
        if x > 4 and x + controlWidth > width - 4 then x, y = 4, y + rowHeight end
  ...
  function viewer:LayoutPalette()
    local width = math.max(20, self.inspector:GetWidth() or 20)
    local columns = math.max(1, math.floor((width - 2) / 20))
    local rows = math.max(1, math.ceil(math.max(1, #(self.paletteItems or {})) / columns))
    self.paletteChild:SetSize(width, rows * 20)
    for index, button in ipairs(self.paletteButtons) do
      local column, row = (index - 1) % columns, math.floor((index - 1) / columns)
      button:ClearAllPoints()
      button:SetPoint("TOPLEFT", self.paletteChild, "TOPLEFT", 1 + (column * 20), -(row * 20))
    end
  end
  ...
  function viewer:LayoutEditorChrome()
    ...
    local actionGap = 3
    local identityWidth = Clamp(math.floor(width * 0.18), 140, 220)
    self.bossName:SetWidth(identityWidth)
    self.planName:SetWidth(identityWidth)

    local x, y = 58 + identityWidth + 8, 10
    local rowRight = width - 10 - (self.close:GetWidth() or 48) - 6
    local function WrapHeaderRow(requiredWidth)
      if x + requiredWidth <= rowRight then return end
      x, y, rowRight = 14, 35, width - 14
    end
    local function PlaceHeaderButton(button)
      button:ClearAllPoints()
      button:SetPoint("TOPLEFT", self, "TOPLEFT", x, -y)
      x = x + (button:GetWidth() or 0) + actionGap
    end
    for index = 1, 5 do
      local button = self.headerActionButtons[index]
      WrapHeaderRow(button:GetWidth() or 0)
      PlaceHeaderButton(button)
    end
    -- Keep the corrected navigation order atomic: Clear, Choose Plan,
    -- Previous, and Next always remain adjacent, wrapping together when the
    -- minimum-size viewer cannot hold the full header on its first row.
    local navigation = { self.clear, self.dropdown, self.previous, self.next }
    local navigationWidth = 0
    for index, button in ipairs(navigation) do
      navigationWidth = navigationWidth + (button:GetWidth() or 0) + (index > 1 and actionGap or 0)
    end
    WrapHeaderRow(navigationWidth)
    for _, button in ipairs(navigation) do PlaceHeaderButton(button) end
    self.headerFlowRows = y > 10 and 2 or 1

    local available = math.max(1, width - VIEWER_HORIZONTAL_CHROME)
    self.toolbarContent:SetSize(available, EDITOR_LAYOUT.toolbarHeight)
    local toolRailWidth = Clamp(math.floor(available * 0.25), 188,
      math.max(188, available - EDITOR_LAYOUT.minimumInspectorWidth - 10))
    local columns = math.max(1, math.floor((toolRailWidth + 2) / 26))
    for index, button in ipairs(self.toolButtons) do
      local column, row = (index - 1) % columns, math.floor((index - 1) / columns)
      button:ClearAllPoints()
      button:SetPoint("TOPLEFT", self.toolbarContent, "TOPLEFT", column * 26, -(row * 26))
    end
    self.inspector:ClearAllPoints()
    self.inspector:SetPoint("TOPLEFT", self.toolbarContent, "TOPLEFT", toolRailWidth + 10, 0)
    self.inspector:SetSize(math.max(1, available - toolRailWidth - 10), EDITOR_LAYOUT.toolbarHeight)
    self:LayoutPalette()
    self:LayoutInspectorControls()
  end
  ...
  for index = 0, 2 do
    ...
    line:SetVertexColor(0.86, 0.58, 0.08, 0.9)
  ...
  SetBackdrop(viewer.dropdownMenu, { 0.02, 0.025, 0.035, 0.99 }, { 0.55, 0.42, 0.18, 1 })
  ...
  SetBackdrop(viewer.confirmation, { 0.025, 0.03, 0.04, 1 }, { 0.9, 0.58, 0.08, 1 })
  ...
  function viewer:UpdateInspector()
    ...
    local show = element ~= nil or creating
    self.inspector:SetAlpha(show and 1 or 0.62)
  ...
  function viewer:UpdateControls()
    self.unlock.label:SetText(self.editing and "Lock Editing" or "Unlock Plan")
  ...
  function viewer:Render(index, preserveDraft)
    ...
    self.dropdown.label:SetText(#self.sourcePlans > 1 and "Choose Plan ▾" or "Single Plan")
...
local function CreateQuickOverview(owner)
  ...
  SetBackdrop(quick, { 0.015, 0.02, 0.028, 0.99 }, { 0.86, 0.58, 0.08, 0.95 })
  ...
  SetBackdrop(quick.close, { 0.03, 0.035, 0.045, 0.96 }, { 0.86, 0.58, 0.08, 1 })
  ...
  local function NavigationButton(label, width)
    ...
    SetBackdrop(button, { 0.03, 0.035, 0.045, 0.96 }, { 0.86, 0.58, 0.08, 1 })
    button.label = button:CreateFontString(nil, "OVERLAY")
...
function MerfinPlus:ShowCurrentRaidAssignmentBossPlan(groupID, selectedBossKey)
  ...
  if #plans == 0 then
    return false, selectedBossKey
      and "No imported MFPRA1 Boss Plan matches the selected boss."
      or "Select a boss with an imported MFPRA1 Boss Plan first."
  end
