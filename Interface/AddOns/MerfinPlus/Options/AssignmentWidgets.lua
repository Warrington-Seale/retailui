...
local colors = {
  ...
  muted = { 0.62, 0.62, 0.58, 1 },
  heading = { 0.12, 0.10, 0.055, 1 },
  row = { 0.045, 0.047, 0.047, 1 },
  gold = { 0.88, 0.74, 0.35, 1 },
...
function MerfinPlus:ShowAssignmentImportDialog(title, onAccept, acceptLabel)
  ...
  if not dialog then
    ...
    if dialog.SetBackdrop then
      ...
      dialog:SetBackdropColor(0.025, 0.028, 0.030, 0.98)
      dialog:SetBackdropBorderColor(0.86, 0.58, 0.08, 0.95)
    end
    dialog.title = dialog:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    dialog.title:SetPoint("TOPLEFT", dialog, "TOPLEFT", 18, -18)
    dialog.title:SetTextColor(1, 0.76, 0.12, 1)
    ...
    if dialog.inputPanel.SetBackdrop then
      ...
      dialog.inputPanel:SetBackdropColor(0.075, 0.065, 0.035, 1)
      dialog.inputPanel:SetBackdropBorderColor(0.86, 0.58, 0.08, 0.95)
    end
    ...
    for _, button in ipairs({ dialog.accept, dialog.cancel }) do
      if button.SetBackdrop then
        ...
        button:SetBackdropColor(0.075, 0.065, 0.035, 0.98)
        button:SetBackdropBorderColor(0.86, 0.58, 0.08, 0.95)
      end
...
local function AcquireAssignmentWidgetRow(index)
  ...
  row.quickPlanHighlight:SetColorTexture(0.95, 0.66, 0.16, 0.12)
...
local function AcquireRaidLeaderWidgetRow(index)
  ...
  row.hoverFill:SetVertexColor(0.12, 0.10, 0.055, 0.96)
  row.hoverFill:SetAllPoints(row)
  row.hoverFill:Hide()
  row.hoverBorder = {}
  local hoverBorderColor = colors.border or colors.gold or { 1, 0.82, 0.25, 1 }
  for _, side in ipairs({ "top", "bottom", "left", "right" }) do
