...
function MerfinPlus:NotifyExportOptionsChanged()
  ...
  if registry then
    ...
    registry:NotifyChange("MerfinPlus_Export")
  end
end
...
function MerfinPlus:BuildGuildExport()
  ...
  if not expansion or not maxLevel then
    return nil, 0, "The current expansion is not supported."
  end
  if not IsInGuild or not IsInGuild() then
    return nil, 0, "No guild detected."
  end

  local memberCount = GetGuildMemberCount()
  if memberCount <= 0 or not GetGuildRosterInfo then
    return nil, 0, "Guild roster data is unavailable."
  end

  local rows = {}
  for index = 1, memberCount do
    local name, rankName, rankIndex, level, className, _, _, _, _, _, classToken = GetGuildRosterInfo(index)
    if name and name ~= "" and tonumber(level) == maxLevel then
      rows[#rows + 1] = {
        name = CleanPlayerName(name),
        class = className or classToken or "Player",
        -- These optional fields keep schema v2 consumers compatible while
        -- preserving the guild's exact rank text and its zero-based index.
...
function MerfinPlus:FinalizeGuildExportRefresh(token, timedOut)
  ...
  local text, count, errorText = self:BuildGuildExport()
  if timedOut and text then
    local _, _, maxLevel = self:GetExportExpansionInfo()
    self:SetGuildExportResult(text, count, errorText, "Guild roster update timed out; %d cached players at level %d.", {
      count,
      maxLevel,
    })
  elseif timedOut then
    self:SetGuildExportResult(text, count, errorText, "Guild roster update timed out; guild data is unavailable.")
  else
...
function MerfinPlus:GenerateGuildExport()
  ...
  if text then
    ...
    self:SetGuildExportResult(text, count, errorText, requested and "Guild roster requested. Waiting for guild data." or nil)
  end
