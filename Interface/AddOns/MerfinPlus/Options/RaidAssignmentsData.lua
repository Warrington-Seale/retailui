...
local BOSS_MEDIA = "Interface\\AddOns\\MerfinPlus\\Media\\assignments\\bosses\\tbc\\"
...
local function RaidBossIcon(raid, sourceName)
  local fileName = string.lower(tostring(sourceName or "")):gsub("[^%w]+", "_"):gsub("_+$", "")
  return BOSS_MEDIA .. raid .. "\\" .. fileName .. ".tga"
end
...
local function RaidBoss(raid, name, sourceName)
  ...
  return {
    ...
    raidKey = raid,
    localeID = raid .. ":" .. key,
    icon = RaidBossIcon(raid, sourceName or name),
  }
end
...
local function NormalizePersistedRaidAssignmentStatus(text)
  ...
  local sender = text:match("^Received MFPRA1 [FDP] revision %d+ from (.-) %(%d+ rows%)%.$")
...
local function ResolveRaidAssignmentSpellIcon(candidates)
  ...
  for _, candidate in ipairs(candidates or {}) do
    if candidate ~= "" then
      local _, _, icon = GetSpellInfo(candidate)
      if icon then
...
local function GetRaidAssignmentSpellIcons(task)
  ...
  return ResolveRaidAssignmentSpellIcon(candidates)
end
...
function MerfinPlus:BuildRaidAssignmentRowDisplay(task, sectionName, playerMap)
  ...
  local target, targetClass, targetIcon, targetIsMarker, targetSpec = self:GetRaidAssignmentTargetDisplay(task, playerMap)
  if assignmentMarkerIcon and targetIsMarker and targetIcon == assignmentMarkerIcon then
    targetIcon, targetIsMarker = nil, false
  end
