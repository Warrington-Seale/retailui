...
local function build()
    ...
    do
        ...
        if layer and okT and type(ids) == "table" and #ids > 0 then
            ...
            for i, fdid in ipairs(ids) do
                ...
                local dx = ((col + 0.5) * layer.tileWidth - cx) * k
                local dy = (cy - (row + 0.5) * layer.tileHeight) * k
                t:SetPoint("CENTER", canvas, "CENTER", dx, dy)
                artTiles[#artTiles + 1] = t
            end
        end
    end
    ...
    -- gap between the room and the south compass letter. Clicking it lands
    -- you where the keys get set
    local noKeys = CreateFrame("Button", nil, canvas)
    local noKeysText = noKeys:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    noKeysText:SetPoint("CENTER")
    noKeysText:SetText("keybinds not set")
    noKeysText:SetTextColor(1, 0.82, 0, 0.9)
    noKeys:SetSize(noKeysText:GetStringWidth() + 6, 14)
    noKeys:SetScript("OnClick", function()
        AZT.OpenOptions()
    end)
    noKeys:SetScript("OnEnter", function(b)
        noKeysText:SetTextColor(1, 0.82, 0, 1)
        GameTooltip:SetOwner(b, "ANCHOR_RIGHT")
        GameTooltip:SetText("Click to open the settings and bind the keys", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
    noKeys:SetScript("OnLeave", function()
        noKeysText:SetTextColor(1, 0.82, 0, 0.9)
        GameTooltip:Hide()
    end)
    noKeys:Hide()
    ...
    -- and the boundaries land on the diagonals.
    ...
    -- hover glow and the press flash both draw with one
