...
local WORD_W = 52 -- room for the widest side word in the small font
...
-- Left column: everything the addon draws, then the sound and the route
...
-- the four voices, menu order. Relative is the recorded one, the rest talk
-- through tts and QuadWord decides what they say
local VOICES = {
...
local function voiceLabel()
    for _, v in ipairs(VOICES) do
        if v.key == AztarecHelperDB.cueVoice then
            return v.label
        end
    end
    return "Relative"
...
local voiceBtn
voiceBtn = addLabelledButton(
    "Cue voice",
    "Relative is the recorded voice calling the move to make, as if you face the boss."
        .. " The rest speak through the game's text to speech and carry no facing at all:"
        .. " Markers names the safe quarter's marker, Colors says its colour, yellow for star,"
        .. " orange for circle and so on with skull staying skull, and Custom says whatever"
        .. " word you typed for the quarter below. The test buttons and the volume follow"
        .. " the choice.",
    100,
    function()
        MenuUtil.CreateContextMenu(voiceBtn, function(_, root)
            root:CreateTitle("Cue voice")
            for _, v in ipairs(VOICES) do
                root:CreateButton(v.label, function()
                    AztarecHelperDB.cueVoice = v.key
                    refreshAll()
                end)
            end
        end)
...
refreshers[#refreshers + 1] = function()
    voiceBtn:SetText(voiceLabel())
end
cueGated[#cueGated + 1] = voiceBtn

-- The Custom voice's word editor, a small box over the pannel with one input
-- per quarter. Saved as typed, so closing it is all there is to do.
local wordsDialog
local function showCueWords()
    if not wordsDialog then
        wordsDialog = CreateFrame("Frame", nil, panel, "BackdropTemplate")
        wordsDialog:SetSize(210, 185)
        wordsDialog:SetPoint("CENTER", UIParent)
        wordsDialog:SetFrameStrata("FULLSCREEN_DIALOG")
        wordsDialog:SetBackdrop({
        ...
        wordsDialog:SetBackdropColor(0, 0, 0, 0.85)
        wordsDialog:EnableMouse(true)
        local head = wordsDialog:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        head:SetPoint("TOP", 0, -12)
        head:SetText("Custom words")
        wordsDialog.rows = {}
        for i, q in ipairs({ "N", "E", "S", "W" }) do
            local icon = wordsDialog:CreateFontString(nil, "OVERLAY", "GameFontNormal")
            icon:SetPoint("TOPLEFT", 16, -18 - i * 26)
            local input = CreateFrame("EditBox", nil, wordsDialog, "InputBoxTemplate")
            input:SetSize(150, 20)
            input:SetPoint("LEFT", icon, "LEFT", 30, 0)
            input:SetAutoFocus(false)
            input:SetMaxLetters(30)
            input:SetScript("OnTextChanged", function(self, user)
                if user then
                    AztarecHelperDB.cueWords[q] = strtrim(self:GetText())
                end
            end)
            input:SetScript("OnEnterPressed", input.ClearFocus)
            input:SetScript("OnEscapePressed", input.ClearFocus)
            wordsDialog.rows[i] = function()
                icon:SetText(AZT.QuadName(q, 16))
                input:SetText(AztarecHelperDB.cueWords[q] or "")
                input:SetCursorPosition(0)
            end
        end
        local done = CreateFrame("Button", nil, wordsDialog, "UIPanelButtonTemplate")
        done:SetSize(80, 22)
    ...
    -- redraw on every open, the quarter icons can have changed under it
    for _, fill in ipairs(wordsDialog.rows) do
        fill()
    end
    wordsDialog:Show()
end

local wordsBtn = addLabelledButton(
    "Custom words",
    "The words the Custom voice says, one per quarter. Numbers, another language, whatever"
        .. " your group calls them. A quarter left blank falls back to its marker name.",
    100,
    showCueWords
)
wordsBtn:SetText("Edit words")
cueGated[#cueGated + 1] = wordsBtn
...
addTip(
    ...
    "For the recorded voice, the sound channel the cues play through, and that channel's"
        .. " volume slider decides how loud they are. For the spoken voices, which of the"
...
-- On the spoken voices the slider is the tts volume instead, the addon's
...
addTip(
    ...
    "How loud the cues are. For the recorded voice this is the game's own volume for the"
        .. " chosen channel, so moving it here moves the sound options too. For the spoken"
        .. " voices it is the text to speech volume, which is the addon's own number."
)
...
local function refreshChannels()
    ...
    chSlider.text:SetText(v)
    chanWarn:SetShown(cuesLive() and not AZT.CueSpoken() and AZT.CueChannelsLow() ~= nil)
end
...
local function applyCueGate()
    ...
    if on and AztarecHelperDB.cueVoice ~= "custom" then
        enableLook(wordsBtn, false, "Locked unless the cue voice is Custom. These are its words.")
    end
    -- the word editor has no business open once its voice is off the menu
    if wordsDialog and not (on and AztarecHelperDB.cueVoice == "custom") then
        wordsDialog:Hide()
    end
    ...
    local chOn = on and not AZT.CueSpoken()
...
addActionPair({
    ...
    tip = "Runs the last recorded route through the room view and the arrow on its real"
        .. " timings. Move along with it or watch, the route stays as it is.",
})
...
addActionPair({
    ...
    tip = "Prints the last recorded route to chat, a ? for any wave you never answered,"
        .. " and which echo got you if you died.",
}, {
    label = "Reset route",
    fn = function()
        AZT.Safe.Reset()
    ...
    tip = "Clears the route on the room view and stops a replay or drill that is running."
        .. " The last pull stays for review and replay.",
})
...
local macroBtn = addLabelledButton(
    "Calling from your bars",
    "Since the August 25 patch, marking and calling only leave your character from real macros"
        .. " on your bars, the keybinds still answer but their marks and calls go silent mid"
        .. " fight. This writes the four, AztCallN through AztCallW, one per quarter: each"
        .. " answers the wave and carries your marking and calling. Drag them on from the macro"
        .. " window's character tab, they keep themselves current after that. Solo, the plain"
        .. " keybinds do everything.",
    120,
    function()
        AZT.WriteCallMacros()
    end
)
macroBtn:SetText("Write macros")
...
local function applyRoleGate()
    ...
    local keysWhy = AztarecHelperDB.relativeTurns and TURNS_WHY
