...
-- The other voices name the safe quarter through text to speech instead,
-- by marker, colour or a word the player typed, no facing assumption in
-- any of them.
...
-- every voice but the recorded one speaks through tts
function AZT.CueSpoken()
    return AztarecHelperDB.cueVoice ~= "relative"
end
...
-- What a quarter is called out loud when the cues name quarters: the word
-- the player typed on the Custom voice, its direction word when the room
-- reads in arrows, else the marker it wears by shape or by colour as the
-- voice says. A blank custom word falls through to the marker name.
function AZT.QuadWord(q)
    if AztarecHelperDB.cueVoice == "custom" then
        local w = AztarecHelperDB.cueWords[q]
        if w and w ~= "" then
            return w
        end
    end
    ...
    local names = AztarecHelperDB.cueVoice == "colors" and MARK_COLORS or MARK_NAMES
...
function AZT.CueChannelWarn()
    if channelsWarned or not AztarecHelperDB.cues or AZT.CueSpoken() then
        return
    end
...
local function speak(safeQuad, fromQuad)
    if AZT.CueSpoken() then
        ...
        if word and not marksWarned then
            marksWarned = true
            AZT.chat("the cue words cannot be spoken on this machine - using the turn words instead")
        end
