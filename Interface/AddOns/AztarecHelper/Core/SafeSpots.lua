...
local function beginCapture(unit)
    ...
    AZT.Log("CAPTURE open - answer each wave")
end
...
local function finishCapture()
    ...
    -- usually still open when the channel stops, so it gets closed here.
...
local function answering()
    return practiceTicker ~= nil or AZT.InDelve()
end
...
local function endPractice(route)
    ...
    for i, q in ipairs(route) do
        if seq[i] == q then
            ...
            -- the misses are the lesson, so each one shows the safe quarter
            -- beside what got answered
            local said = seq[i] and seq[i] ~= "?" and AZT.QuadName(seq[i], 14) or "nothing"
            wrong[#wrong + 1] = ("wave %d was %s, you answered %s"):format(i, AZT.QuadName(q, 14), said)
        end
    end
    AZT.chat(("practice sermon over, %d of %d answered right - playing your recording back"):format(right, #route))
    if #wrong > 0 then
        AZT.chat(table.concat(wrong, ", "))
    end
    snapshotPull()
    setWave(nil, 0, nil, nil)
...
function Safe.Practice()
    ...
    -- the lead-in counts down in the wave box, chat keeps the one line
    setWave("lead", 0, nil, GetTime() + PRACTICE_LEAD)
    practiceTicker = C_Timer.NewTimer(PRACTICE_LEAD, startDrill)
end
