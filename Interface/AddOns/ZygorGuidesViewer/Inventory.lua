...
local function isvalidsellabegray(itemID)
	...
	local _, _, quality, _, _, _, _, _, _, _, price, classID, subclassID, _, _, _, _, _ = ZGV:GetItemInfo(itemID)
...
function Inventory:QueueMoveItems(source,itemid,count)
	...
	if Inventory.QueueTimer then ZGV:CancelTimer(Inventory.QueueTimer) end
	Inventory.QueueTimer = ZGV:ScheduleRepeatingTimer(function() 
...
function Inventory:QueueMoveItemsHandler()
	...
	if #Inventory.Queue==0 then
		ZGV:CancelTimer(Inventory.QueueTimer)
	end
end
