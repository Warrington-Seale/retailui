...
local function getSpellName(spellID)
	local subtext = ZGV.IsClassic and GetSpellSubtext and GetSpellSubtext(spellID)
	local name
	...
	return (name or "?")..(subtext and " "..subtext or "")
end
...
GOALTYPES['get'] = { -- combining old 'get' and 'collect' now.
	...
	iscomplete = function(self)
		...
		local got,bank,bags = 0,0,0

		if self.targets then
			for _,target in ipairs(self.targets) do
				if tonumber(target[2]) then
					local intotal = C_Item.GetItemCount(target[2],self.usebank)
					bags = bags + intotal
					got = got + intotal
					if self.usebank then
						local inbags = C_Item.GetItemCount(target[2])
						bags = bags - intotal + inbags
						bank = bank + (intotal-inbags)
					end
				end
			end
		else
			if tonumber(self.targetid) then
				local intotal = C_Item.GetItemCount(self.targetid,self.usebank)
				got = got + C_Item.GetItemCount(self.targetid,self.usebank)
				bags = bags + intotal
				if self.usebank then
					local inbags = C_Item.GetItemCount(self.targetid)
					bags = bags - intotal + inbags
					bank = bank + (intotal-inbags)
				end
			end
	...
	onevent = function(self, _, event, interactionType)
		...
		if (self.force_nocomplete or not GOALTYPES['get'].iscomplete(self)) and ((event=="PLAYER_INTERACTION_MANAGER_FRAME_SHOW" and interactionType==Enum.PlayerInteractionType.Banker) or BankFrame:IsVisible()) then 
			...
			ZGV.Inventory:QueueMoveItems("bank",self.targetid,targetcount)
		end
	end,
...
GOALTYPES['bank'] = {
	...
	onevent = function(self, _, event, interactionType)
		...
		if (event=="PLAYER_INTERACTION_MANAGER_FRAME_SHOW" and interactionType==Enum.PlayerInteractionType.Banker) or BankFrame:IsVisible() then 
			ZGV.Inventory:QueueMoveItems("bags",self.targetid,"all")
		end
	end,
}
...
GOALTYPES['learnmount'] = {
	parse = function(self,params)
		...
		if ZGV.IsRetail or ZGV.IsClassicCATA or ZGV.IsClassicMOP then
			...
			self.mountname = mountID and C_MountJournal.GetMountInfoByID(mountID) 
		end
	...
	gettext = function(self) return L["stepgoal_learnmount"]:format(COLOR_ITEM(self.mountname or getSpellName(self.spellid) or self.spell)) end,
}
...
GOALTYPES['learnspell'] = {
	...
	iscomplete = function(self)
		return C_SpellBook.IsSpellKnown(self.spellid,Enum.SpellBookSpellBank.Player), true
	end,
...
GOALTYPES['learnpetspell'] = {
	...
	iscomplete = function(self)
		return C_SpellBook.IsSpellKnown(self.spellid,Enum.SpellBookSpellBank.Pet), true
	end,
...
GOALTYPES['turnin'] = {
	...
	iscomplete = function(self)
		...
		local turned = ZGV.completedQuests[self.questid]
					and (not quest or not quest.inlog)

		local completed = quest and quest.inlog -- if 0 goals, we are ready
		
		-- 20260722 - disable actual completion checks. if quest is in log, turnin should be possible
...
GOALTYPES['turninany'] = {
	parse = function(self,params)
		GOALTYPES['_item'].parse(self,params)
		if not self.targets then self.targets = {{self.target,self.targetid}} end
		self.multiquests = self.targets
		self.multiquest = self.target
		self.multiquestid = self.targetid
		self.targetid = nil
		self.targets = nil
		self.target = nil
	end,
	iscomplete = function(self)
		...
		local any_relevant = false

		for _,data in ipairs(self.multiquests) do
			local questID = data[2]
			local quest = ZGV.questsbyid[questID]
			local turned = ZGV.completedQuests[questID] and (not quest or not quest.inlog)

			-- "ready to turn in right now": in log, goals satisfied, not yet turned
			local completed = quest and quest.inlog
			if quest and quest.goals then
			...
			local relevant = (quest and quest.inlog) or turned
			if relevant then
				any_relevant = true
				complete = complete and turned
			end
		...
		complete = complete and any_relevant
		possible = possible or complete

		return complete, possible, 0, 0
	end,
	...
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief) 
		local title = (type(self.quest)=="string" and self.quest) or (type(self.quest)=="table" and self.quest.title) or Lretrydots(self)
		return (brief and "%s" or L["stepgoal_turn in"..(complete and "_done" or "")]):format(COLOR_QUEST(L['questtitle']:format(title,self.questpart))) 
	end,
}
			...
			if self.objnum then
				...
				if not self.action or self.action=="" or self.action=="complete" then
					...
					if self.accountwide then
						return C_QuestLog.IsQuestFlaggedCompletedOnAccount(self.questid), true, nil,nil, "quest completion"
					else
...
GOALTYPES['stablemaster'] = {
	...
	events = {"PET_STABLE_SHOW","PET_STABLE_CLOSED"},
	onenterevent = "PET_STABLE_SHOW",
	onevent = function(self, _, event, ...)
		ZGV:ScheduleTimer(function()
			if PetStableFrame and PetStableFrame:IsVisible() then -- from TRAINER_SHOW and INIT
				self.propernpc = (ZGV.GetUnitId("NPC")==self.npcid)
			end
		end,0)
	...
	iscomplete = function(self)
		return self.actioncompleted,self.required or ZGV.db.profile.showstablemaster
	end,
	skippable = function(self)
		return not ZGV.db.profile.showstablemaster and not self.required and self.skipoptional
	end,
	onenter = function(self)
		self.actioncompleted = false
		self.propernpc = false
	end,
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief) return (brief and "%s" or L["stepgoal_stablemaster"..(complete and "_done" or "")]):format(COLOR_NPC(plural(ZGV.Localizers:GetTranslatedNPC(tonumber(self.useid or self.npcid),self.usename or self.npc),self.plural))) end,
}
...
GOALTYPES['expcollect'] = {
	parse = function(self,params)
		self.params=params
		self.item,self.turnincount,self.turningain,self.targetlevel = params:match("^(.*),(%d*),(%d*),(%d*)$")
		self.turnincount = tonumber(self.turnincount)
		self.turningain = tonumber(self.turningain)
		self.targetlevel = tonumber(self.targetlevel)
		GOALTYPES['_item'].parse(self,self.item)
	end,
	iscomplete = function(self)
		...
		if level>=self.targetlevel then
			self.count = 0
			return true,true
		end
		...
		for lvl = (level + 1), self.targetlevel do
		    totalXPNeeded = totalXPNeeded + (ZGV.ExpToLevel[lvl] or 0)
		end

		totalXPNeeded = totalXPNeeded - UnitXP("player")		

		self.count = math.ceil(totalXPNeeded/self.turningain) * self.turnincount
	...
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief) 
		...
		if complete then 
			return L["stepgoal_collectexp_done"]:format(COLOR_ITEM(itemname),self.targetlevel)
		else
			local goalcountnow = C_Item.GetItemCount(self.targetid or 0)
			if self.targets then
				goalcountnow = 0
			...
			if remaining<1 then remaining=self.count end
			local progtext = L["completion_goal"]:format(goalcountnow,self.count)			
			return L["stepgoal_collectexp"]:format(COLOR_COUNT(remaining),COLOR_ITEM(itemname),self.targetlevel),nil,progtext
		end
	end,
	help = "expcollect <item##12345>,<count per turnin>,<exp per turnin>,<target level> -- completes when user has farmed enough items to reach level via X turnins granting Y exp each. "
}
...
GOALTYPES['havebuff'] = {
	...
	iscomplete = function(self)
		if not InCombatLockdown() then
			...
			self.numdone = aura and math.max(aura.applications,1) or 0
		end
		
		local iscomplete = self.numdone >= self.count
			
		return iscomplete,true,self.numdone,self.count
	end,
	gettext = function(self)
		if self.spellid and not self.qspellname then self.qspellname = getSpellName(self.spellid) end
		if self.count>1 then
			return L["stepgoal_havebuff_count"]:format(self.count-self.numdone, plural(L["stepgoal_havebuff_stack"],self.count-self.numdone), COLOR_ITEM(self.qspellname or self.spell or self.texture))
		else
...
GOALTYPES['fly'] = {  -- obsolete, JFTR
	...
	iscomplete = function(self)
		if not UnitOnTaxi("player") then
			...
			local epicflyer=C_SpellBook.IsSpellKnown(34091,Enum.SpellBookSpellBank.Player) or IsSpellKnown(90265) -- Artisan and Master riding respectively
...
function Goal:IsComplete()
	...
	if self.action=="accept" or self.action=="turnin" then
		...
		if self.condition_complete then
			return complete,true,norm_nums(complete),nil,"quest a/t with complete"
		else
			return complete,possible,norm_nums(complete),nil,"quest a/t regular" 
		end
	end
	...
	if self.questid and self.action~="abandon" then
		...
		if iscomplete or (not self.future and (not ispossible or self.objnum) and not (self.action=="get" and ispossible)) then  
			return iscomplete,ispossible,numdone,numneeded,comment,isbad
		else
			fallthrough_ispossible,fallthrough_numdone,fallthrough_numneeded = ispossible,numdone,numneeded
		end
	...
	if GOAL and GOAL.iscomplete and (not GOAL.iscompletable or GOAL.iscompletable(self)) then
		...
		if self.action=="get" and self.usebank and iscomplete then
			-- banked gets keep their numbers if completed, overriding fallbacks
			fallthrough_numdone = numdone
			fallthrough_numneeded = numneeded
		end
		numdone,numneeded = norm_nums(numdone,numneeded)
		if not numdone and not numneeded then  numdone,numneeded = iscomplete and 1 or 0,1  end
...
function Goal:AutoTranslate()
	...
	if self.questid or self.multiquestid then
		local qid = self.questid or self.multiquestid
		local q = ZGV.Localizers:GetQuestData(qid)
		if q then
			self.quest=q
	...
	if self.L then
		...
		if qid then self.Lreasons = self.Lreasons .. " questid "..qid end
...
function Goal:GetText(showcompleteness,brief,showtotals,nocolor)
	...
	if showcompleteness and not self.quests then
		if numdone and numneeded then remaining = numneeded-numdone end -- get from goal, if possible, first
	...
	elseif self.action=='get' or self.action=='craft' or self.action=='farm' then
		...
		if self.bank then
			local bags = goalcountnow - self.bank
			local frombank = 0
			
			if (goalcountneeded-self.bags) > 0 then
				frombank = math.min(goalcountneeded-self.bags,self.bank)
			end
			
			if frombank > 0 then
				extramsg = L["stepgoal_get_bank"]:format(self.bank)
			--elseif self.bank > 0 then
			--	extramsg = L["stepgoal_get_inbank"]:format(self.bank)
			end
		end
	...
	if showtotals and not (self.displaystyle=="hidden") then
		if not progtext and not (goalcountneeded==1) then
	...
	text = text .. (extramsg and " "..extramsg or "")
