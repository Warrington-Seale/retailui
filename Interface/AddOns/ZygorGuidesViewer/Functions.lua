...
function ZGV.IsItemBound(bagID, bagSlotID)
	...
	local warbound = C_Item.IsBoundToAccountUntilEquip and C_Item.IsBoundToAccountUntilEquip(item_location)
	local playerbound = C_Item.IsBound(item_location)
	return warbound or playerbound,warbound or quality==item_quality_heirloom or quality==item_quality_wowtoken
end
...
function ZGV.WillBeSecret(type)
	if not C_Secrets then return false end
	
	if type=="aura" then return C_Secrets.ShouldAurasBeSecret() end

	ZGV:Debug("Unknown secret type queried "+(type or "???"))
end
