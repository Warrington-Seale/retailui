...
local ConditionEnv = {
	...
	hasbuff = function(query,count)
		...
		if spell and C_UnitAuras then -- wotlk doesn't have C_UnitAuras, so we need to fall back to looping
			...
			if aura and (math.max(aura.applications or 0,1))>=(count or 1) then return true end
			return false
		else
			if ZGV.WillBeSecret("aura") then return end
	...
	completedq = function(...)
		...
		local IsQuestCompleted = goal and goal.accountwide and C_QuestLog.IsQuestFlaggedCompletedOnAccount or C_QuestLog.IsQuestFlaggedCompleted
		
		local count = select("#", ...)
		for i = 1, count do
			local id = select(i, ...)
			if tonumber(id) then -- just quest
				if IsQuestCompleted(id) then return true end
			else 
				if id:match("-") then -- range of quests
					...
					local qcomplete = IsQuestCompleted(id)
	...
	completedallq = function(...)
		...
		for i = 1, count do
			local id = select(i, ...)
			if not IsQuestCompleted(id) then return false end
		end
		return true
	end,
	countcompletedq = function(...)
		...
		for i = 1, count do
			local id = select(i, ...)
			if IsQuestCompleted(id) then res = res + 1 end
		end
		return res
	end,
	...
	achieveprogress = function(achieveid,subid) -- return progress within achievement
		-- if only achieveid is givem returns completed criteria count
		if type(achieveid)=="number" and not subid then 
			local completedachieve = select(4,GetAchievementInfo(achieveid)) 
			local num = GetAchievementNumCriteria(achieveid)
			if completedachieve then
				return num
			end
			...
			for i=1,num do
				local desc, ctype, completed, quantity, requiredQuantity, charName = ZGV.Zygor_GetAchievementCriteriaInfo(achieveid,i)
				if completed then numcompletedcriteria=numcompletedcriteria+1 end
			end
			return numcompletedcriteria
		end
		-- if criteria id is given, returns progress within given criteria
		if type(achieveid)=="number" and type(subid)=="number" then 
			local desc, ctype, completed, quantity, requiredQuantity, charName = ZGV.Zygor_GetAchievementCriteriaInfo(achieveid,subid)
			return quantity
		end
	...
	itembanked = function(...)
		...
		for i = 1, count do
			local itemid = select(i, ...)
			local bags = C_Item.GetItemCount(itemid)
			local withbank = C_Item.GetItemCount(itemid,true)
			local bank = withbank-bags
			total = total + bank
		end
		return total
	end,
	...
	traitknown = function(treeID,nodeID,entry)
		local configID = C_Traits.GetConfigIDByTreeID(treeID)
		if not configID then return false,"no configID" end
		local nodeInfo = C_Traits.GetNodeInfo(configID, nodeID)
		if not nodeInfo then return false,"no nodeInfo" end
		
		local active = nodeInfo.activeEntry and nodeInfo.activeEntry.entryID
		return active==nodeInfo.entryIDs[entry]
	end,

	vaultextrataxis = function()
		return Parser.ConditionEnv.traitknown(1191,110463,2)
	end,	
			...
			for chunk in line:gmatch("%s*(.-)%s*|+") do if #chunk>0 then
				...
				if step and not cmd_parsed then
					...
					elseif cmd=="delay" then
						-- use if step should linger after completing or becoming impossible
						step.delay = tonumber(params)
						
					elseif cmd=="orlogic" then
						local fun,err = loadstring(("return (%s)"):format(params))
						if fun then
							step.orlogic = fun
						end
					...
					--]]
						...
						elseif cmd=="or" then
							if not params or params=="" or tonumber(params) then
								goal.orcount = tonumber(params) or 1
								goal.orgroup = ("orgroupdefault%d"):format(goal.orcount)
							else
								local count,group = params:match("(%d+)%s+(.+)")
								goal.orgroup = group
								goal.orcount = tonumber(count)
							end
						...
						elseif cmd=="accountwide" then
							goal.accountwide = true
							
						elseif cmd=="forceuse" then
							goal.forceuse = true
