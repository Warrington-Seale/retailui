...
function ActionBar:Initialise()
	...
	if ZGV.postcombatmode then ActionBar:SetActionButtons() end -- on classic, init will run after PLAYER_REGEN_ENABLED, so natural startup will not trigger. force refesh now.
end
...
function ActionBar:SetActionButtonsQueued()
	...
	for gi,goal in ipairs(goals) do
		if goal:IsVisible() and not goal:IsComplete() then
			...
			elseif goal.action=="talk" and goal.npcid and ZGV.db.profile.actionbar_talk and not actions_npc_lookup[goal.usename or goal.npc] then
			...
			elseif goal.action=="clicknpc" and goal.npcid and ZGV.db.profile.actionbar_talk and not actions_npc_lookup[goal.usename or goal.npc] then
	...
	for _,data in ipairs(actions) do 
		...
		if not usedactions[data[1]][data[2]] then 
			counter = counter + 1
			ZGV.ActionBar:SetButton(data[1],data[2],data[3],counter,nil,data.desaturated) 
	...
	for _,data in ipairs(actions_npc) do 
		...
		if not usedactions[data[1]][data[2]] then 
			counter = counter + 1
			ZGV.ActionBar:SetButton(data[1],data[2],data[3],counter,data.sticky,data.desaturated)
			usedactions[data[1]][data[2]] = true
		end
	end
