...
local function _StartupThread()
	...
	ZGV.postcombatmode = ZGV:IsPlayerInCombat()
	local function run_startup_cycle(phase)
		local done=0
		for i,startup in ipairs(self.startups) do  if not startup.done then
			if (not startup.postcombat or phase=="postcombat" or not ZGV.postcombatmode) and (not startup.after or all_done(startup.after)) then
...
function ZGV:FindEvent(eventName)
	...
	for event=1, numEvents do
		...
		if ZGV.IsSecret(eventdata) or ZGV.IsSecret(eventdata.calendarType) then return false end
...
function ZGV.ClearRaidmarker()
	if ZGV.skipping then return end	
	if ZGV.IsRetail then return end
