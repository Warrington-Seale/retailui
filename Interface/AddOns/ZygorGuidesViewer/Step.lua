...
function Step:IsComplete(novisibilitychecks)
	...
	if not self:AreRequirementsMet("iscomplete","strict") then return true,true,"req unmet" end
	...
	-- prepare statuseses
	if not novisibilitychecks then
	...
	-- set initial values for orgroups
	local orgroups_status = {}
	local orlogic_exists = false

	-- update status of all orgroups
	for i,goal in ipairs(self.goals) do
		...
		if status~="hidden" then
			...
			if goal.orgroup then
				orlogic_exists = true
				orgroups_status[goal.orgroup] = (orgroups_status[goal.orgroup] or goal.orcount)
				if status=="complete" then
					orgroups_status[goal.orgroup] = orgroups_status[goal.orgroup] - 1
				end
			end
		end
	end
	...
	if orlogic_exists then
		-- set all orgroups_status to boolean
		for group,count in pairs(orgroups_status) do
			orgroups_status[group] = count<1
		end
		
		if self.orlogic then
			-- logic is defined, run it with orgroups_status as enviroment
			setmetatable(orgroups_status,{__index=function(t,k) local lower=rawget(t,k:lower())  if lower~=nil then return lower end  return _G[k]  end})
			setfenv(self.orlogic,orgroups_status)
			orcomplete = self.orlogic()
		else
			-- if no logic is specified, we default to "any group done"
			for group,groupstatus in pairs(orgroups_status) do
				if groupstatus then 
					orcomplete=true
				end
			end
	...
	for i,goal in ipairs(self.goals) do
		...
		if status=="complete" or status=="incomplete" or status=="warning" then
			completeable = true
			local done,possible
			if goal.orgroup and orcomplete then
...
function Step:AreRequirementsMet(case,strict)
	...
	if self.condition_visible then
		...
		if not self.condition_visible() then 
			-- we know that condition is not met
			-- properly fail if there is no delay, or if we are skipping 
			if not self.delay or ZGV.completionstreak>1 then return false,"condition fail" end
			
			-- otherwise start ticking the delay down until we can remove it (only from iscomplete, so we avoid all side-checks)
			if self.delay and ZGV.CurrentStep==self and case=="iscomplete" then 
