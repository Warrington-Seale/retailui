...
function Widgets.UpdateDriver(elapsed)
	...
	if Widgets.ConfigMode then
		...
		elseif Widgets.Parent:IsMouseOver(-1,1,1,-1) then -- skip 1px margin, since it may not be used by widgets due to grid being math.floored
			...
			for ident,object in pairs(Widgets.Registered) do
				if object.active and not object.system and object.frame:IsMouseOver(padding,-padding,-padding,padding) then 
					is_over_widget=true 
					break 
				end
			end
			...
			if ZGV.Widgets.Parent.ConfigButton:IsMouseOver(padding,-padding,-padding,padding) then
				is_over_widget=true 
			end
