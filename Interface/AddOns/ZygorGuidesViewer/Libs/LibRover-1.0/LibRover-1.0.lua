...
local function _StartupThread()
	...
	if Lib.IsRetail or Lib.IsClassicMOP then
		...
		Lib.frame:RegisterEvent("UNIT_EXITED_VEHICLE")
		Lib.frame:RegisterEvent("UNIT_ENTERING_VEHICLE")
		Lib.frame:RegisterEvent("UNIT_ENTERED_VEHICLE")
	end
...
function Lib:SetupInitialQuickTravel(startnode)
	...
	for i,port in ipairs(Lib.data.portkeys) do repeat
		...
		local reject_reason = (function()
			...
			if (UnitOnTaxi("player") or self.unitOnTaxi) then return "no keying the taxi" end
		...
		then
			...
			-- === FAILED NODES 
				...
				if Lib.IsRetail or Lib.IsClassicCATA then
					...
					if mapinfo_src and mapinfo_dst and ((mapinfo_src.mapType==4 and mapinfo_dst.mapType~=4) or (mapinfo_src.mapType==4 and mapinfo_dst.mapType~=4)) then
...
local function onEvent(this, event, arg1, arg2, arg3, arg4, arg5)
	...
	elseif event == "UNIT_ENTERED_VEHICLE" then
		...
		if (issecretvalue and issecretvalue(arg5)) then return end
		if arg1~="player" then return end
		
		-- vault of atalurek pseusotaxis
		local unitID = tonumber(arg5:match("Vehicle%-%d+%-%d+%-%d+%-%d+%-(%d+)"))
		if unitID and special_taxi_npcs[unitID] then	
			Lib.unitOnTaxi=true
			LibTaxi:PlayerOnOffTaxi(true)
			Lib:UpdateNow()
		end
	elseif event == "UNIT_EXITED_VEHICLE" then
		Lib.unitOnTaxi=false
		LibTaxi:PlayerOnOffTaxi(false)
		Lib:UpdateNow()
	...
	elseif event=="UNIT_SPELLCAST_SUCCEEDED" then
		...
		Lib.Cantrips:DepartingFor(arg3)
	...
	elseif event=="ZONE_CHANGED" or event=="ZONE_CHANGED_NEW_AREA" or event=="ZONE_CHANGED_INDOORS" or event=="LOADING_SCREEN_DISABLED" then
		if event=="LOADING_SCREEN_DISABLED" then
			...
			if not (UnitOnTaxi("player") or Lib.unitOnTaxi) then
...
do -- There and back again spell handler
	local Cantrips = {}
	Lib.Cantrips = Cantrips
	
	function Cantrips:Init()
		Cantrips.ExitDestination = {}
		if not Lib.data.cantrips then return end
		Cantrips.Data = Lib.data.cantrips
		
		for spellID,data in pairs(Cantrips.Data) do
			local exitnode = Lib.nodes.id[data.id]
			if exitnode and ZGV.db.char[data.id] then
				exitnode:Update(unpack(ZGV.db.char[data.id]))
			end
			Cantrips.ExitDestination[spellID] = exitnode
		end
	end

	function Cantrips:Ready(spellID)
		local exitnode = Cantrips.ExitDestination[spellID]
		if not exitnode then return false end

		-- only suggest this spell if we are on its home map
		local x,y,m = LibRover:GetPlayerPosition()
		if not m then return end

		for _,mapID in ipairs(Cantrips.Data[spellID].maps) do
	...
	function Cantrips:DepartingFor(spellID)
		local exitnode = Cantrips.ExitDestination[spellID]
		if not exitnode then return false end

		if not Cantrips.Data[spellID] then return false end

		-- fires when spell is used, but player is still on source map
		local x,y,m = LibRover:GetPlayerPosition()
		if not m then return end

		for _,mapID in ipairs(Cantrips.Data[spellID].maps) do
			if m==mapID then return false end
			-- we are on spells home map, do not update coords
		end
		
		exitnode:Update(m,x,y)
		ZGV.db.char[Cantrips.Data[spellID].id] = {m,x,y}
	end
	
	tinsert(Lib.startup_modules_funcs,{"Setting up Cantrips",function(self)	
		self.Cantrips:Init()
	end})
