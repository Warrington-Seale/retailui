...
do
	...
	function Lib:Startup(newsave)
		...
		ZGV:AddMessageHandler("ZGV_GOSSIP_PICKED",Lib.OnEvent)
		...
		local current_wow_version = GetBuildInfo()
		ZGV.db.global.taxitimes[current_wow_version] = ZGV.db.global.taxitimes[current_wow_version] or {}
		Lib.TaxiTimesSV = ZGV.db.global.taxitimes[current_wow_version]
	...
	function Lib:GetMapContinent(mapID)
		...
		if cont==13 then extracont={2509} end -- EK needs to also scan vault of nararlek
		...
		if mapID==2599 then cont_scan=2622 cont=2537 end -- val>voidstorm
		if mapID==2600 then cont_scan=2623 cont=2537 end -- nagital>voidstorm
		
		-- vault of nararlek is connected to EK network, but is only visible via its own map
		if mapID==2509 then extracont={2509} end -- vault of nararlek > ek
	...
	function Lib:PlayerHasGuildPerk()
		return IsSpellKnown(117983)
	end
		...
		if ontaxi then
			...
			if taxis and taxis[1] and taxis[1][2]<50 then
				...
				if not fc_array[dep_ident].neighbors or not fc_array[dep_ident].neighbors[arr_ident] then
					...
					Lib.TaxiTimesSV[dep_ident.."__"..arr_ident]=triptime
					return
				end
				...
				if Lib:GetCurrentMapContinent()==ENUM_CONTINENT_KHAZALGAR and Lib:PlayerHasKhazAlgarFlightMasterAchievement() then -- flight time was 25% reduced
					triptime = triptime * 1.33
				end
				
				if Lib:PlayerHasGuildPerk() then -- flight time was 25% reduced
					triptime = triptime * 1.25
				end
				
				fc_array[dep_ident].neighbors[arr_ident]=triptime
				Lib.TaxiTimesSV[dep_ident.."_"..arr_ident]=triptime
	...
	function Lib.OnEvent(this, event, arg1,arg2)
		...
		elseif event=="UNIT_FLAGS" and arg1=="player" and LibRover.ready then
			local ontaxi = UnitOnTaxi("player") or LibRover.unitOnTaxi
		...
		elseif event=="TAXI_NODE_STATUS_CHANGED" then
			Lib:MarkKnownTaxis()
		elseif event=="ZGV_GOSSIP_PICKED" then
			Lib:TakeGossipTaxi(arg1)
		end
	end
	...
	function Lib:CacheTaxiPoints()
		for c,z,n,node in ZGV.CoroPairs(Lib.taxipoints,IterateCZNN) do
			if not is_enemy(playerF,node.faction) then
				...
				if node.gossipIDS then 
					for gossipID,destID in pairs(node.gossipIDS) do
						Lib.connections_by_gossipID[gossipID] = {from=node.taxinodeID,to=destID}
					end
				end
				node._zone = z
			end
	...
	function Lib:ImportTaxiTimesFromSV()
		...
		process(ZGV.db.global.taxitimes)
	end
	...
	function Lib:TakeGossipTaxi(gossipID)
		local gossipConnection = Lib.connections_by_gossipID[gossipID]
		if gossipConnection then
			local start = Lib:FindTaxiByNodeID(gossipConnection.from)
			local dest = Lib:FindTaxiByNodeID(gossipConnection.to)
			
			Lib.LastTaxi = {fullname=dest.name,node=dest}
			Lib.LastTaxi.name,Lib.LastTaxi.zone = Lib.LastTaxi.node.name,ZGV.GetMapNameByID(Lib.LastTaxi.node.m)
			Lib.LastTaxi.route = {start,dest} -- gossip routes are direct point to point connections
			
			local time = Lib:GetTaxiTripTime(gossipConnection.from,gossipConnection.to)
		
			Lib.LastTaxi.eta = time or -1
			Lib.LastTaxi.departure = GetTime()
			
			Lib:Debug("Gossip takeoff hook, estimated time %d",time)
		end
	end
	...
	function Lib:DEV_DumpFlightCosts(onlycont)
		...
		for contnum,contdata in ZGV.OrderedPairs(Lib.flightcost) do  repeat
			...
			for i,data in ipairs(contdata) do
				...
				for k,v in pairs(data) do if k~="taxioperator" and k~="faction" and k~="comment" and k~="name" and k~="neighbors" and k~="tag" and k~="nodeID" then print(data.nodeID,"Odd data key",k) end end
