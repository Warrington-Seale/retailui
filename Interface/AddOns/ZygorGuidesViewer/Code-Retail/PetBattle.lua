...
tinsert(ZGV.startups,{"PetBattle hooks",function(self)
	...
	if (IsFrameLockActive and IsFrameLockActive("PETBATTLES")) or (UIModeUtil and UIModeUtil.IsModeActive("PetBattle")) then
...
function PetBattle:MainFrameUpdate()
	...
	local bottomPoint = (pet.Suggest3 and pet.Suggest3:GetBottom())
			 or (pet.Suggest2 and pet.Suggest2:GetBottom())
			 or (pet.Suggest1 and pet.Suggest1:GetBottom())
...
function PetBattle:ShowInterface()
	...
	local topVal    = PetBattle.BattleFrame.Main.top:GetTop()
	local bottomVal = PetBattle.BattleFrame.Main.bottom:GetBottom()
	if topVal and bottomVal then
	    SetHeightAllFrames(topVal + 15 - bottomVal)
	end
end
