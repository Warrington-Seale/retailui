...
function Appraiser:ActivateBuyItem(item)
	...
	AuctionHouseFrame.SearchBar:SetSearchText('"'..item.name..'"')
...
function Appraiser:SelectedResult(item)
	...
	if (item.itemKey.itemID==Appraiser.ActiveItem.itemid) or (item.itemKey.battlePetSpeciesID==breed) then
		...
		AuctionHouseFrame.CommoditiesBuyFrame.BuyDisplay.QuantityInput.InputBox:SetText(count)
		AuctionHouseFrame.CommoditiesBuyFrame.BuyDisplay.QuantityInput.InputBox:OnTextChanged(1)
	end
end
...
function Appraiser:SearchForBuyItem(item)
	...
	local result = ZGVG.Scan:ScanByName(item.name,item.itemid,"dontanalyze")
	item.updated = time()
	item.pending = true
	return result
end
