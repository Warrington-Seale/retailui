...
function QuestItem:GetQuestRewardIndex()
	...
	for index=1, totalrewards do 
		...
		local _, _, _, _, _, _, _, _, _, _, sellPrice, classID, subclassID, _, _, _, _ = ZGV:GetItemInfo(itemlink)
		local _,_,itemStackCount,_,_ = GetQuestItemInfo("choice",index)
		...
		if (sellPrice*itemStackCount) > best_sell_value then
			best_sell_value = (sellPrice*itemStackCount)
			best_sell_index = index
			ZGV:Debug(index.." is new best by selling price "..best_sell_value)
		end
	end
