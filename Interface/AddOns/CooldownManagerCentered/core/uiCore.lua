...
local function WatchAnchoredFrame(frame, configKey)
    ...
    if not anchoredFrameWatchTicker then
        anchoredFrameWatchTicker = C_Timer.NewTicker(ANCHORED_FRAME_WATCH_INTERVAL, function()
            local toRemove = {}
            for watchKey, entry in pairs(anchoredFrameWatch) do
                local ok, tickerError = pcall(function()
                    ...
                    if targetExists ~= entry.targetExists then
                        ...
                        local okPosition, positionError = pcall(WilduUICore.ApplyFramePosition, entry.frame, entry.configKey)
                ...
                if not ok then
                    ns.API:LogWarn(ns.API.Error.AnchorWatchDrop, "AnchoredFrameWatch: dropping watch " .. tostring(watchKey) .. ": " .. tostring(tickerError))
                    toRemove[#toRemove + 1] = watchKey
                end
            end
...
function WilduUICore.ApplyFramePosition(frame, configKey)
    ...
    local anchorAnythingActive = ns.Anchoring and ns.Anchoring:IsAnchorAnythingActive(frame)
    if not anchorAnythingActive then
        frame:SetClampedToScreen(true)
    end

    -- Position ownership is strict: Anchor Anything first, then a configured
    -- CMC anchor, and finally the stored Edit Mode/UIParent position.
    local anchored = false
    local hasConfiguredAnchor = ns.Anchoring and ns.Anchoring:HasTarget(configKey)
    if hasConfiguredAnchor then
        ns.Anchoring:WatchFrame(frame, configKey)
        if not anchorAnythingActive then
            WatchAnchoredFrame(frame, configKey)
            anchored = ns.Anchoring:ApplyAnchor(frame, configKey)
        end
    elseif ns.Anchoring and not anchorAnythingActive then
        ns.Anchoring:ReleaseAnchor(frame, configKey)
    end

    if not anchorAnythingActive and not anchored and not hasConfiguredAnchor then
...
function WilduUICore.CreateOnPositionChanged(configKey)
    return function(frame, layoutName, point, x, y)
        if ns.Anchoring and ns.Anchoring:IsAnchorAnythingActive(frame) then
            return
        end
        ...
        WilduUICore.ApplyFramePosition(frame, configKey)
    end
end
...
function WilduUICore.RegisterEditModeCallbacks(frame, configKey, enabledCheckFn, shouldRepositionFn)
    frame._wt_editModeEnabledCheck = enabledCheckFn

    LEM:RegisterCallback("enter", function()
        local enabled = enabledCheckFn()
        if frame._wt_VisibilityDriver and not InCombatLockdown() then
            ...
            frame._wt_VisibilityDriver = nil
        elseif enabled and not frame._wt_VisibilityDriver and not frame:IsShown() then
            frame._wt_hideOnEditModeExit = true
        end
        if not shouldRepositionFn or shouldRepositionFn() then
            WilduUICore.ApplyFramePosition(frame, configKey)
        end
...
function WilduUICore.RegisterFrameWithLEM(
    ...
    defaultTable.editModeEnabled = function()
        local enabledCheckFn = frame._wt_editModeEnabledCheck
        local anchorAnythingActive = ns.Anchoring and ns.Anchoring:IsAnchorAnythingActive(frame)
        return AreCustomFramesEnabled() and not anchorAnythingActive and (not enabledCheckFn or enabledCheckFn())
    end
