...
local observedHostGeometry = setmetatable({}, { __mode = "k" })
...
local function snapWithPixelScale(value, scale, factor, minPixels)
...
function PixelPerfect.SyncContainer(host)
    ...
    local factor = PixelPerfect.GetPixelToUIUnitFactor()
    local observed = observedHostGeometry[host]
    if
        observed
        and observed.width == width
        and observed.height == height
        and observed.left == left
        and observed.top == top
        and observed.scale == scale
        and observed.factor == factor
    then
        return container
    end
    if not observed then
        observed = {}
        observedHostGeometry[host] = observed
    end
    observed.width, observed.height = width, height
    observed.left, observed.top = left, top
    observed.scale, observed.factor = scale, factor

    local offsetX, offsetY = 0, 0
    if left ~= nil then
        offsetX = snapWithPixelScale(left, scale, factor) - left
    end
    if top ~= nil then
        offsetY = snapWithPixelScale(top, scale, factor) - top
    end

    local snappedWidth = snapWithPixelScale(width, scale, factor, width > 0 and 1 or nil)
    local snappedHeight = snapWithPixelScale(height, scale, factor, height > 0 and 1 or nil)
