...
EventHandler.events["PLAYER_ENTERING_WORLD"] = function()
    C_Timer.After(1, function()
        if EditModeManagerFrame.accountSettings ~= nil and not InCombatLockdown() then
            C_EditMode.SetAccountSetting(Enum.EditModeAccountSetting.ShowBossFrames, 0)
            C_EditMode.SetAccountSetting(Enum.EditModeAccountSetting.ShowCooldownViewer, 1)
            C_EditMode.SetAccountSetting(Enum.EditModeAccountSetting.ShowEncounterEvents, 0)
        end
    end)
end
