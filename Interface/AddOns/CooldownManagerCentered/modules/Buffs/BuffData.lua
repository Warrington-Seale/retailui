    ...
    for _, category in ipairs(categories) do
        ...
        if type(cooldownIDs) == "table" then
            for _, cooldownID in ipairs(cooldownIDs) do
                ...
                AddCooldownInfoSpellIDs(blizzardAuraSpellIDs, C_CooldownViewer.GetCooldownViewerCooldownInfo(cooldownID))
            end
        end
    end
    return blizzardAuraSpellIDs, blizzardAuraCooldownIDs
...
function BuffData.GetShowingUnusable()
    return BuffData.GetDB().showUnusable == true
end

function BuffData.ToggleShowUnusable()
    local db = BuffData.GetDB()
    db.showUnusable = not BuffData.GetShowingUnusable()
...
local function GetTrackedBuffCooldownIDs()
...
function BuffData.ContainerHasBlizzardAura(containerIndex)
    return ContainerHasBlizzardAuraRaw(containerIndex)
end

---Buffs configured in Blizzard's Tracked Buff Icons category, including
---currently unknown/unavailable entries that do not have live viewer frames.
...
function BuffData.ScanTrackedBuffs(force)
    ...
    local entries = {}
    local seen = {}
    for layoutIndex, cooldownID in ipairs(GetTrackedBuffCooldownIDs()) do
        if not seen[cooldownID] then
            ...
            if info and info.stableKey then
                ...
                info.layoutIndex = layoutIndex
                info.unavailable = cooldownInfo and cooldownInfo.isKnown == false or nil
    ...
    for _, specDB in pairs(root.bySpec or {}) do
        ...
        if assignments then
            for stableKey in pairs(assignments) do
                if type(stableKey) == "string" and stableKey:sub(1, 2) == "w:" and not wildcardDefinitions[stableKey] then
...
local function SortEntries(entries)
    ...
    table.sort(
        entries,
        ns.API:BuildOrderComparator(function(entry)
            return orders[entry.stableKey]
        end, function(entry)
            return entry.layoutIndex
        end, function(entry)
            return entry.stableKey or ""
        end)
    )
end
...
function BuffData.SortCooldownFrames(frames)
    ...
    table.sort(
        frames,
        ns.API:BuildOrderComparator(function(frame)
            return orders[BuffData.GetStableKeyForCooldownID(frame:GetCooldownID())]
        end, function(frame)
            return frame.layoutIndex
        end, function(frame)
            return frame:GetCooldownID() or 0
        end)
    )
end
...
function BuffData.ReconcileContainerCount()
    ...
    for stableKey, index in pairs(db.assignments) do
        if (customAurasEnabled or not BuffData.IsCustomKey(stableKey)) and type(index) == "number" and index > maxUsed and index <= MAX_CONTAINERS then
            maxUsed = index
        end
    end
