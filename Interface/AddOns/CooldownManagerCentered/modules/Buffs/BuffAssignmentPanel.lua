...
local function InitializeButton(button)
    ...
    button:SetScript("OnEnter", function(self)
        ...
        if entry and entry.spellID then
            ...
            elseif entry.unavailable then
                GameTooltip:AddLine(_G.UNAVAILABLE or "Unavailable", 0.5, 0.5, 0.5)
            end
            GameTooltip:Show()
    ...
    button:HookScript("OnShow", function(self)
        ...
        elseif self.buffEntry and self.buffEntry.custom then
            ...
            self.Icon:SetDesaturated(self.buffEntry.untracked == true or self.buffEntry.unavailable == true)
        end
    end)
...
local function LayoutCategory(category, entries, searchActive)
    ...
    if #entries == 0 and not searchActive then
        ...
        for index, entry in ipairs(entries) do
            ...
            button.Icon:SetDesaturated(entry.untracked == true or entry.unavailable == true)
            if entry.custom then
                ns.CooldownStyle.HideSettingsItemIndicator(button)
            else
...
local function RefreshPanelInternal()
    ...
    local showUnusable = BuffData.GetShowingUnusable()
    local function filter(entries)
        local result = {}
        for _, entry in ipairs(entries) do
            local name = entry.name and entry.name:lower()
            local usable = showUnusable or (not entry.unavailable and not entry.untracked)
            local matchesSearch = not searchTerm or (name and name:find(searchTerm, 1, true))
            if usable and matchesSearch then
    ...
    for i = 1, #containerCategories do
        ...
        if i <= count then
            ...
            elseif BuffData.ContainerHasBlizzardAura(i) then
                title = string.format(L["Buffs %d (Blizzards)"], i)
            end
            category.Header:SetHeaderText(title)
...
function BuffAssignmentPanel:EnsureSettingsTab()
    ...
    local settingsDropdown = CreateFrame("DropdownButton", nil, CMCCooldownViewerSettingsBuffPanel, "UIPanelIconDropdownButtonTemplate")
    settingsDropdown:SetPoint("LEFT", searchBox, "RIGHT", 5, 0)
    settingsDropdown:SetupMenu(function(_, rootDescription)
        rootDescription:CreateCheckbox(L["Show Unusable"], BuffData.GetShowingUnusable, BuffData.ToggleShowUnusable)
    end)
    settingsDropdown:Hide()
    CMCCooldownViewerSettingsBuffPanel.buffSettingsDropdown = settingsDropdown
