...
local function LayoutBaseBuffRow(icons, total, forceLayout, nativeTotal)
    ...
    if forceLayout then
        ...
        for _, entry in ipairs(ns.BuffData.GetUnassignedBuffs()) do
            -- Unknown tracked buffs are available in the assignment panel, but
            -- have no native frame and therefore must not reserve layout slots.
            if not entry.untracked and not entry.unavailable then
