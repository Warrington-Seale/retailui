...
-- Unique CMC-owned chain hosts that need their bounds refreshed (frame -> true).
local activeChainHosts = setmetatable({}, { __mode = "k" })
...
local function SyncGroups(container, newDefinitions, auraTypeFilter)
    container.definitions = container.definitions or {}
    ...
    if not auraTypeFilter then
        ReplaceDefinitions(container, newDefinitions)
    end

    for stableKey, definition in pairs(newDefinitions) do
        local auraType = GetAuraType(definition)
        if not auraTypeFilter or auraType == auraTypeFilter then
            local blocked = IsAuraTypeBlocked(auraType)
            local controller = container.entryControllers[stableKey]
            local unitToken = GetUnitToken(auraType)
            if controller and container.entryUnits[stableKey] ~= unitToken then
                controller:SetUnit(unitToken)
            end
            container.entryUnits[stableKey] = unitToken
            if blocked then
    ...
    if not auraTypeFilter then
        for stableKey, controller in pairs(container.entryControllers) do
            if not newDefinitions[stableKey] then
                ...
                controller:SetEnabled(false)
                controller:Hide()
                container.entryUnits[stableKey] = nil
            end
        end
...
-- Combat-safe host wrap: sizes the CMC-owned host around the current chain.
-- Never touches the protected AuraContainer
-- controllers, so it can run while combat lockdown is active.
local function ResizeChainHost(host)
    if not host then
        return
    end
    host:SetSize(0.001, 0.001)
...
function CustomAuraProvider:ChainContainers(container)
    ...
    if InCombatLockdown() then
        ...
        ResizeChainHost(container.chainHost)
        return
    end
    local host = container.chainHost
    if not host then
    ...
    for index, stableKey in ipairs(items) do
        ...
        elseif horizontal then
            if firstEdge == "LEFT" then
                controller:SetPoint("LEFT", previous, "RIGHT", 0)
            else
                controller:SetPoint("RIGHT", previous, "LEFT", 0)
            end
        elseif firstEdge == "TOP" then
            controller:SetPoint("TOP", previous, "BOTTOM", 0, 0)
        else
            controller:SetPoint("BOTTOM", previous, "TOP", 0, 0)
        end
        previous = controller
    end
    ResizeChainHost(container.chainHost)
...
function CustomAuraProvider:RefreshContainer(container, newDefinitions)
    ...
    if container.chainHost then
        activeChainHosts[container.chainHost] = true
    end
    if Runtime.IsVehicleAuraContainerBlocked() then
        ReplaceDefinitions(container, newDefinitions)
...
function CustomAuraProvider:DisableContainer(container)
    ...
    if container.chainHost then
        local hostStillActive = false
        for activeContainer in pairs(activeContainers) do
            if activeContainer.chainHost == container.chainHost then
                hostStillActive = true
                break
            end
        end
        if not hostStillActive then
            activeChainHosts[container.chainHost] = nil
        end
    end
    for stableKey, controller in pairs(container.entryControllers or {}) do
        if container.groupKeys and container.groupKeys[stableKey] then
...
local function ResizeAllChainHosts()
    for host in pairs(activeChainHosts) do
        ResizeChainHost(host)
    end
end

local targetRefreshTicker
local function RefreshAuraContainers()
    if not next(activeContainers) then
        return
    end
    ...
    for container in pairs(activeContainers) do
        local restoreAll = container.vehicleAuraControllersSuspended == true
        if restoreAll then
            SyncGroups(container, container.definitions)
            CustomAuraProvider:ApplyGroupLayout(container)
            CustomAuraProvider:ChainContainers(container)
        elseif SyncGroups(container, container.definitions, "helpful") then
            CustomAuraProvider:ApplyGroupLayout(container)
            CustomAuraProvider:ChainContainers(container)
        end
        ResizeChainHost(container.chainHost)
...
local function refreshTargetAuras()
    ...
    for container in pairs(activeContainers) do
        for stableKey, controller in pairs(container.entryControllers or {}) do
            if (container.entryUnits or {})[stableKey] == "target" and (container.definitions or {})[stableKey] then
                controller:UpdateAllAuras()
            end
        end
    end
...
local auraLayoutRefresh = CreateFrame("Frame")
auraLayoutRefresh:RegisterUnitEvent("UNIT_AURA", "player")
auraLayoutRefresh:RegisterUnitEvent("UNIT_AURA", "target")
auraLayoutRefresh:RegisterEvent("PLAYER_TARGET_CHANGED")
auraLayoutRefresh:SetScript("OnEvent", function(self, event)
    if not next(activeContainers) then
        return
    end
    ...
    if targetRefreshTicker then
        targetRefreshTicker:Cancel()
    end
    refreshTargetAuras()
    targetRefreshTicker = C_Timer.NewTicker(0.03, refreshTargetAuras, 2)
end)
