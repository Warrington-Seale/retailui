...
local function ResetLockedState()
    if EX_DB then
        EX_DB.locked = true
    end
end
...
local function SaveAnchorPosition(frame)
    ...
    EX_DB.posX = RoundToNearest((frameCenterX * frameScale - uiCenterX * uiScale) / uiScale)
    EX_DB.posY = RoundToNearest((frameCenterY * frameScale - uiCenterY * uiScale) / uiScale)
...
local function UpdateAnchorClampState()
    if not anchorFrame or not anchorFrame.SetClampedToScreen then
        return
    end

    local shouldClamp = not (isEditModeActive and isEditModeVisible)
    anchorFrame:SetClampedToScreen(shouldClamp)
end

local function StartAnchorMove(frame)
    if not frame then return end
    if not ((isEditModeActive and isEditModeVisible) or isPreviewing or not EX_DB.locked) then
        return
    end
...
local function EX_RegisterLayout()
    local layout = {
        ...
        { key = "div1", type = "divider", x = 1, y = 6, w = 53, h = 1, label = L["--[[ Function ]]"] },
        { key = "subheader_general", type = "subheader", x = 1, y = 5, w = 53, h = 1, label = L["通用设置"], labelSize = 20 },
        { key = "enabled", type = "checkbox", x = 1, y = 7, w = 8, h = 2, label = L["启用"] },
        { key = "showBar", type = "checkbox", x = 11, y = 7, w = 10, h = 2, label = L["显示施法条"] },
        { key = "playSound", type = "checkbox", x = 1, y = 20, w = 10, h = 2, label = L["播放提示音"] },
        { key = "muteSoundOnInterruptCD", type = "checkbox", x = 13, y = 20, w = 18, h = 2, label = L["打断CD时不播放音效"] },
        { key = "locked", type = "checkbox", x = 23, y = 7, w = 8, h = 2, label = L["锁定位置"] },
        { key = "preview", type = "checkbox", x = 34, y = 7, w = 8, h = 2, label = L["预览"] },
        { key = "btn_reset_pos", type = "button", x = 36, y = 14, w = 16, h = 2, label = L["重置位置"] },
        { key = "posX", type = "slider", x = 1, y = 14, w = 15, h = 2, label = L["位置 X"], min = -1000, max = 1000 },
        { key = "posY", type = "slider", x = 19, y = 14, w = 15, h = 2, label = L["位置 Y"], min = -1000, max = 1000 },
        { key = "subheader_sound", type = "subheader", x = 1, y = 17, w = 53, h = 1, label = L["音效设置"], labelSize = 20 },
        { key = "sound", type = "lsm_sound", x = 1, y = 24, w = 15, h = 2, label = L["选择音效"], labelPos = "top" },
        { key = "soundChannel", type = "dropdown", x = 19, y = 24, w = 14, h = 2, label = L["输出频道"], items = { { L["主音量"], "Master" }, { L["效果"], "SFX" }, { L["环境"], "Ambience" }, { L["音乐"], "Music" }, { L["对话"], "Dialog" } }, labelPos = "top" },
        { key = "btn_test_sound", type = "button", x = 36, y = 24, w = 16, h = 2, label = L["测试音效"] },
        { key = "subheader_bar", type = "subheader", x = 1, y = 34, w = 53, h = 2, label = L["施法条设置"], labelSize = 20 },
        { key = "showInterruptMarkerLine", type = "checkbox", x = 1, y = 38, w = 17, h = 2, label = L["显示打断技能CD转好的线条"] },
        { key = "interruptMarkerColor", type = "color", x = 20, y = 38, w = 12, h = 2, label = L["线条颜色"], labelPos = "top" },
        { key = "interruptMarkerWidth", type = "slider", x = 34, y = 38, w = 20, h = 2, label = L["线条粗细"], min = 1, max = 8 },
        { key = "hideOnInterruptCD", type = "checkbox", x = 1, y = 42, w = 17, h = 2, label = L["打断CD时隐藏可断条"] },
        { key = "showInterruptCDThreshold", type = "slider", x = 34, y = 42, w = 20, h = 2, label = L["打断CD剩余几秒时显示(左边颜色)"], min = 0, max = 10 },
        { key = "nonInterruptColor", type = "color", x = 1, y = 46, w = 17, h = 2, label = L["无法打断颜色"], labelPos = "top" },
        { key = "interruptCDColor", type = "color", x = 20, y = 42, w = 12, h = 2, label = L["打断CD时颜色"], labelPos = "top" },
        { key = "textAlign", type = "dropdown", x = 7, y = 81, w = 15, h = 2, label = L["法术对齐"], items = "LEFT,CENTER,RIGHT", labelPos = "left", labelSize = 18 },
        { key = "showTarget", type = "checkbox", x = 29, y = 132, w = 8, h = 2, label = L["显示目标"], labelSize = 18 },
        { key = "targetAlign", type = "dropdown", x = 39, y = 132, w = 15, h = 2, label = L["目标对齐"], items = "LEFT,CENTER,RIGHT", labelPos = "top", labelSize = 15 },
        { key = "showTimer", type = "checkbox", x = 1, y = 132, w = 10, h = 2, label = L["显示时间"], labelSize = 18 },
        { key = "timerAlign", type = "dropdown", x = 12, y = 132, w = 12, h = 2, label = L["时间对齐"], items = "LEFT,CENTER,RIGHT", labelPos = "top", labelSize = 15 },
        { key = "hideWhenNotInterruptible", type = "checkbox", x = 1, y = 10, w = 19, h = 2, label = L["|cffff080a隐藏不能打断的条 (隐藏钢条)|r"], labelSize = 18 },
        { key = "timerGroup", type = "timerBarGroup", x = 1, y = 50, w = 53, h = 27, label = L["计时条外观"], labelSize = 20 },
        { key = "font_spell_header", type = "header", x = 1, y = 78, w = 53, h = 2, label = L["法术文字设置"], labelSize = 20 },
        { key = "font_spell", type = "fontgroup_v10", x = 1, y = 84, w = 53, h = 43, label = L["法术名称"], labelSize = 20 },
        { key = "font_target_header", type = "header", x = 1, y = 128, w = 53, h = 2, label = L["目标文字设置"], labelSize = 20 },
        { key = "font_target", type = "fontgroup_v10", x = 1, y = 136, w = 53, h = 43, label = L["施法目标"], labelSize = 20 },
        { key = "font_timer_header", type = "header", x = 1, y = 180, w = 53, h = 2, label = L["时间文字设置"], labelSize = 20 },
        { key = "font_timer", type = "fontgroup_v10", x = 1, y = 183, w = 54, h = 42, label = L["剩余时间"], labelSize = 20 },
        { key = "divider_7144", type = "divider", x = 1, y = 18, w = 53, h = 1, label = L["新组件"] },
        { key = "divider_5422", type = "divider", x = 1, y = 36, w = 53, h = 1, label = L["新组件"] },
        { key = "customSoundPath", type = "input", x = 1, y = 29, w = 53, h = 2, label = L["使用自定义路径 (如留空则默认使用上面选单的音效)"] },
        { key = "description_5178", type = "description", x = 1, y = 31, w = 53, h = 2, label = L["|cffafafaf输入路径: (举例) Interface\\AddOns\\Exwind\\sound\\注意打断.mp3|r"], labelSize = 14 },
        { key = "description_6169", type = "description", x = 20, y = 46, w = 34, h = 2, label = L["举例:打断CD剩余2秒时会显示左边颜色的条 打断CD好的瞬间会变色"], labelSize = 14 },
    }
...
local function UpdateInterruptMarker(bar, castInfo)
    ...
    bar.InterruptMarkerBar:SetFillStyle(castInfo.isChanneling and Enum.StatusBarFillStyle.Reverse or
        Enum.StatusBarFillStyle.Standard)
...
local function RefreshAnchorState()
    ...
    local shouldShowHandle = (isEditModeActive and isEditModeVisible) or
        ((not isEditModeActive) and (isPreviewing or (not EX_DB.locked)))
...
function TogglePreview(enable)
    ...
    if enable then
        ...
        if previewBar then
            ...
            -- 预览模式下始终保留示例条，避免 showBar 关闭后编辑模式无可拖拽目标。
...
ExwindTools:WatchState(EXWIND_MODULE_KEY .. ".ButtonClicked", EXWIND_MODULE_KEY, function(info)
    ...
    if info.key == "btn_reset_pos" then
        EX_DB.posX = EX_DEFAULTS.posX or 0
        EX_DB.posY = EX_DEFAULTS.posY or 0
        if anchorFrame then
            anchorFrame:ClearAllPoints()
...
ExwindTools:RegisterEvent("PLAYER_LOGOUT", EXWIND_MODULE_KEY, function()
    ResetLockedState()
end)
