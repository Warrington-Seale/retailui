...
local function Init_CombatAlert()
    ...
    enterText:SetFont(ExwindTools.MAIN_FONT or "Fonts\\ARHei.ttf", 48, "THICKOUTLINE")
    ...
    enterFrame:SetScript("OnDragStop", function(self)
        ...
        if cx and sx then
            db.CombatEnterPosX = RoundToNearest(cx - sx)
            db.CombatEnterPosY = RoundToNearest(cy - sy)
    ...
    leaveText:SetFont(ExwindTools.MAIN_FONT or "Fonts\\ARHei.ttf", 48, "THICKOUTLINE")
    ...
    leaveFrame:SetScript("OnDragStop", function(self)
        ...
        local sx, sy = UIParent:GetCenter()
        if cx and sx then
            db.CombatLeavePosX = RoundToNearest(cx - sx)
            db.CombatLeavePosY = RoundToNearest(cy - sy)
            if ExwindTools.UI and ExwindTools.UI.MainFrame and ExwindTools.UI.MainFrame:IsShown() then
                ExwindTools.UI:RefreshContent()
            end
        end
    ...
    local function RefreshCombatAlertOverlay(frame, moduleKey, title, visible)
        if isEditModeActive and visible then
            ExwindTools:ShowExwindToolsEditOverlay(moduleKey, frame, {
                title = title,
                ownerFrame = frame,
            })
        else
    ...
    local function ApplySingleFrameEditPresentation(frame, moduleKey, title, visible)
        if isEditModeActive and visible then
            ...
            frame:Show()
            frame:SetAlpha(1)
            RefreshCombatAlertOverlay(frame, moduleKey, title, true)
        else
            frame:SetMovable(false)
            frame:EnableMouse(false)
            frame.bg:Hide()
            frame:Hide()
            RefreshCombatAlertOverlay(frame, moduleKey, title, false)
        end
    end

    local function RefreshCombatAlertEditPresentation()
        ApplySingleFrameEditPresentation(enterFrame, "ExTools.MiniTools.CombatAlert.Enter", L["进入战斗提示"], isEnterVisible)
        ApplySingleFrameEditPresentation(leaveFrame, "ExTools.MiniTools.CombatAlert.Leave", L["离开战斗提示"], isLeaveVisible)
    end
