...
local StartMainFrameMove, StopMainFrameMove
...
StartMainFrameMove = function(frame)
    if not frame or EX_DB.locked then
        return
    end
...
StopMainFrameMove = function(frame)
    if not frame or not frame.moving then
        return
    end
...
ExwindTools:WatchState(EXWIND_MODULE_KEY .. ".ButtonClicked", EXWIND_MODULE_KEY, function(info)
    ...
    if info.key == "btn_reset" or info.key == "btn_reset_pos" then
        ResetPosition()
