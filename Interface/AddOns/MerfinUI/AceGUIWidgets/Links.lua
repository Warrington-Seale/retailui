...
local widgetType = 'MerfinUILinks'
...
local function getFontObject(size)
  local fontObject = fontObjects[size]

  if not fontObject then
    fontObject = E:GenerateFontObject('MerfinUILinksFont', mediaFont, size, appliedFontStyle)
    E:SetFontShadow(fontObject, appliedFontStyle, false)
    fontObjects[size] = fontObject
  end
...
local function applyFont(fontInstance, size)
  ...
  if fontInstance.SetScaleAnimationMode then
...
local function rebuildFontObjects()
  wipe(fontObjects)
  mediaFont = E.media.normFont or STANDARD_TEXT_FONT
  useFontSlug = E:CanFlagSlug(fontStyle)
...
local function Constructor(AceGUI, data)
  rebuildFontObjects()
...
function Private.RegisterLinksWidget(data)
  ...
  if (AceGUI:GetWidgetVersion(widgetType) or 0) < widgetVersion then
    AceGUI:RegisterWidgetType(widgetType, function()
      return Constructor(AceGUI, data)
    end, widgetVersion)
  end
